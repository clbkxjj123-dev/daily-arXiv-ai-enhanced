...
TL;DR: 本文提出AnatoProto，一个轻量级序列级框架，用于在低成本产科盲扫超声中检测胎儿腹围标准平面。该框架冻结BiomedCLIP编码器，通过解剖加权空间池化、用例内原型损失、三级级联精炼和混合预测头四个组件，在ACOUSLIC-AI基准上达到67.72的测试F1，超过最强基础模型基线13.20个F1点。


<details>
  <summary>Details</summary>
Motivation: 在低成本产科盲扫超声中检测胎儿腹围标准平面是一个高度类别不平衡的帧分类问题：正样本帧占比不足3%，形成短连续片段，且现成的超声和视觉基础模型处理效果不佳，需要专门设计轻量级序列级解决方案。

Method: AnatoProto包含四个组件：(i)解剖加权空间池化，利用nnU-Net腹部区域概率作为空间先验重加权BiomedCLIP补丁令牌； (ii)用例内原型损失，将每个帧嵌入拉向同一扫描中正样本帧的均值；(iii)三级级联精炼（帧->片段->用例级拒绝器），将预测单元从噪声帧提升到结构约束的片段；(iv)混合预测头，联合建模帧内稳定性和帧间边界转换以抑制边界假阳性。

Result: 在ACOUSLIC-AI基准上，AnatoProto达到67.72的测试F1，较最强基础模型基线FetalCLIP+PRS（F1=54.52）提升+13.20 F1，较最强视频时序动作检测基线TriDet+PRS提升+15.76 F1。联合消融研究发现原型损失和解剖加权池化不是简单叠加关系：单独应用原型损失使召回率降低12个点，但结合解剖加权池化后反而使召回率提升6.5个点，这一符号翻转可追溯至用例内原型的准确性。

Conclusion: AnatoProto通过利用解剖语义先验和用例级结构信息，有效解决了低质量盲扫超声中标准平面检测的极端类别不平衡问题，显著超越了基础模型和视频时序动作检测基线，为低成本胎儿超声筛查提供了高效方案。
...
TL;DR: 本文系统综述了2024-2026年间手术视频生成领域的文献，将其划分为无条件生成、条件生成和世界模型生成三大类，揭示了该领域从合成视觉上可信帧到建模手术场景因果动态的根本性转变，并指出泛化性、物理真实性、可控性和可解释性是当前的核心瓶颈。


<details>
  <summary>Details</summary>
Motivation: 手术视频数据是术中感知、手术工作流理解和机器人决策模型的主要训练资源，但临床数据获取受限于隐私、成本和类别不平衡等问题。手术视频生成作为一种应对数据稀缺的变革性方法，其发展迅速却缺乏清晰的概念框架，亟需系统性梳理。

Method: 对2024-2026年的相关文献进行系统调研，将现有方法分为三类：无条件生成（unconditional generation）、条件生成（conditional generation）和世界模型生成（world modeling generation），并总结代表性方法在公开数据集上的实验结果，提供定量参考。

Result: 揭示了手术视频生成任务定义的根本性转变——从合成视觉上可信的帧转向建模手术场景的因果动态；识别出像素级保真度与临床可信度之间的持续差距；确认泛化性、物理真实性、可控性和可解释性为领域发展的主要瓶颈。

Conclusion: 该综述为从事智能感知、多模态融合、生成式AI和手术数据科学交叉研究的研究者提供了当前研究现状与开放性挑战的结构化参考。
...
TL;DR: 提出Procedura，一个将3D形状表示为程序化组装代码的建模代理框架。该框架利用LLM的编码能力，将单张图像或文本提示转化为带参数、可编辑、可机器校验装配关系、且有材质和关节的3D程序化模型，在质量、锐利边缘和可编辑性上全面超越SOTA原生3D生成器及先前的3D代码代理。


<details>
  <summary>Details</summary>
Motivation: 原生3D生成器虽然能恢复网格几何，但存在三大缺陷：密集网格在机械物体的尖锐边缘处保持柔软、没有部件分解、不暴露任何用户可编辑的参数。这些缺陷阻碍了3D生成模型在工程和设计场景中的实用化，因此需要一种能输出结构化、可编辑、带部件语义的3D表示范式的探索。

Method: 提出"3D形状即代码"范式，构建Procedura代理框架：(1) 从文本提示规划对象为装配图并逐步编写程序化组装代码；(2) 每个部件通过类型化、机器可检查的"mate"（配合约束）从配合坐标系求解位姿，而非猜测；(3) 仅当编译、配合和连通性检查全部通过时才接受部件；(4) 解耦的视觉批评者以诊断-修复循环逐步细化装配；(5) 装配图携带每部件材质及经模拟器验证的关节信息。

Result: 在P3D-Bench及其装配判定器上，以及自建的MechBench-36硬表面基准上，Procedura在判定质量上优于SOTA原生3D生成器和所有先前3D代码代理；在所有评估方法中产生最锐利的边缘；且是唯一输出可编辑、带部件结构的程序的方法。

Conclusion: 程序化组装代码是3D建模的一个有前景的范式方向。通过将LLM的编码能力扩展至3D领域，并以机器可校验的配合约束和视觉批评者循环来保证正确性，不仅能生成更高质量的3D几何（尤其针对硬表面机械物体），还能同时获得部件分解、参数可编辑性和可模拟的关节结构，弥补了原生3D生成器在结构化输出方面的根本不足。
...
TL;DR: 本文提出Modality Maturity Index (MMI)——一个跨五种模态（文本、图像、音频、视频、文档）及其最多三模态组合的多模态能力评估基准，包含893个带人工评分标准的问题。对五个前沿多模态模型评估后发现模态存在评分（MPS）仅15.6-34.9，表明模型输出模态生成能力严重不足，故以MPS为主要结果；单独的LLM评委与盲评人工标注一致性达70.8%。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型被宣称为跨模态的"全能系统"，但现有评估框架几乎只关注双模态理解（通常为文本加另一种模态），缺乏对多模态输入输出组合能力的系统性评估。

Method: 提出MMI基准，覆盖五模态及至多三模态的输入输出组合，包含893道自包含问题，每题带有针对每种预期输出模态的人工编写评分标准。引入MMI Value（各模态得分平均值）和Modality Presence Score (MPS)（预期输出模态上的逐题F1）。使用LLM judges配合评分标准进行评估，并单独设计了与盲评人工标注的对比实验。

Result: 对五个前沿多模态模型评估，MPS仅从15.6（Claude Opus 4.6）到34.9（GPT-5.4），表明模型返回预期输出模态的能力非常有限；LLM评委与盲评人工标注者的一致性为70.8%。

Conclusion: 当前前沿模型在多模态生成（尤其是跨三模态组合）方面能力严重不足，MPS作为主要结果反映了"连可评分的模态都很少返回"的现状；LLM评委配合评分标准进行评判的方式显示出了一定可行性，未来需待模型能力提升后进一步评估MMI Value。
...
TL;DR: PACE是一个因子引导的长视频证据获取框架，采用两阶段策略：先基于问题派生的因子索引片段级描述（不观察候选答案），再用候选答案推导对比线索进行验证。在MMR-V上达到42.6%的准确率，优于直接推理和先前的agentic基线（如DVD），并在LVBench、Video-MME、EgoSchema和LongVideoBench上一致超越DVD。


<details>
  <summary>Details</summary>
Motivation: 长视频问答仍具挑战性：相关证据稀疏，问题相关上下文常无法提供区分正确答案与看似合理选项的线索。对MMR-V手动标注子集的诊断分析显示，先前agentic系统在检索线索上显著优于直接VLM推理，但答案准确率未获得相应提升，表明瓶颈在于选项判别性证据而非主题相关性。

Method: PACE采用两阶段渐进式证据获取：(1) 索引构建与因子引导检索阶段——基于问题派生的因子索引片段级描述，此阶段不观察候选答案；(2) 对比验证阶段——利用候选答案推导对比线索并查询索引进行验证。

Result: 在MMR-V上（配合开源Qwen3-VL骨干）达到42.6%的准确率，优于直接推理和先前agentic基线（包括DVD）；在同一诊断子集上恢复了66.9%的标注线索，证明性能提升源自证据恢复改进而非仅靠答案侧先验。在LVBench、Video-MME、EgoSchema和LongVideoBench上一致超越DVD。

Conclusion: 选项感知的证据获取策略是可迁移的，超出MMR-V数据集之外也有效。PACE的增益主要与改进的证据恢复相关联，而非仅依赖答案侧的强先验。代码已开源。
...
TL;DR: 本文提出了一个统一的数学框架，将CNN中的信息传播与物理空间描述（相对论能量-动量关系、热方程/扩散方程、薛定谔方程及Friedmann方程）联系起来。作者建立了离散滤波器对称性与相对论能量-动量关系之间的对应：对称滤波器分量等价于静止能量mc²（保持图像质心），反对称分量等价于动量项pc（引起位移）。反复滤波产生高斯尺度空间和尺度不变特征，共享Laplacian驱动结构，并在3D图像（从糖分子到CMB）中展示出blob状Morse临界点。


<details>
  <summary>Details</summary>
Motivation: 连接信息空间与物理空间的描述，为CNN的信息传播机制提供一个统一的数学理论基础，揭示深度学习中尺度不变性、扩散过程等现象与物理系统中关键方程之间的深层结构相似性。

Method: 通过数学对应关系建立离散滤波器对称性与相对论能量-动量关系的映射：对称分量（如Σ=[1,1]）类比mc²，反对称分量（如∇=[-1,1]）类比pc，位移由反对称与总滤波器能量之比β=pc/E决定；将重复滤波操作与热方程、薛定谔方程及Friedmann方程的Laplacian驱动结构对应；在3D图像中验证Morse拓扑临界点。

Result: 证明了小离散滤波器的位移由反对称与总能量之比决定（类Lorentz变换）；重复滤波产生高斯尺度空间和尺度不变特征；在多种物理尺度的3D图像（糖分子、硅晶体、人脑和灵长类脑MRI、星系、CMB）中均发现blob状、尺度不变的Morse临界点。

Conclusion: CNN的卷积信息传播与物理空间的核心方程（相对论能量-动量、热方程、薛定谔方程、Friedmann方程）之间存在统一的结构对应关系，这一框架可为理解和设计神经网络提供物理学视角的理论基础。
...
TL;DR: 本文提出VIPER，这是首个由兽医病理学专家精心策划的毒理病理学视觉语言模型评估基准，包含419张H&E染色大鼠组织学图像上的1,251个问题，跨七个器官系统，涵盖选择题、KPrim和自由文本格式。基准测试了16个模型（包括2个新引入的兽医病理学模型），发现兽医与人类病理学之间存在显著的领域差距，前沿模型存在对正常组织的过度诊断风险，且领域特定训练对视觉基础预测至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有病理学视觉语言模型基准集中于人类组织（尤其是肿瘤学），忽视了非人类病理学领域。毒理病理学中实验室动物的微观组织检查是临床前药物安全评估的核心组成部分，因此需要专门的评估基准来填补这一空白。

Method: 作者构建了VIPER基准：包含419张H&E染色大鼠组织学图像，跨七个器官系统，共1,251个问题，涵盖多项选择、KPrim和自由文本三种格式。所有问题由获得委员会认证的兽医病理学家策划和验证。在此基础上，对16个模型进行了基准测试，包括2个新引入的兽医病理学模型、7个人类病理学专业模型和7个通用前沿模型。

Result: 基准测试结果显示兽医病理学与人类病理学之间存在显著的领域差距；前沿模型存在对正常组织过度诊断的风险；领域特定训练对于视觉基础预测仍然至关重要。

Conclusion: VIPER填补了毒理病理学评估基准的空白，为推进兽医病理学视觉语言模型的发展提供了标准化的评估工具和公开资源。
...
TL;DR: 本文提出一个基于数据融合技术的深度学习框架，用于在新南威尔士州进行木本植被（高度超过2米）分割。通过图像合成法、预测融合法和多源标签迁移，分别将误差降低38.2%、53.6%和28.1%-76.2%，显著提升了模型的数据效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 树木覆盖图是遥感领域的基础产品，但高质量无云无缺陷影像获取困难，而深度学习模型虽能生成高质量地图却依赖大量人工标注数据。本研究旨在解决深度学习在植被分割中面临的数据效率低和对图像质量鲁棒性差两大问题。

Method: 提出三部分框架：(1) 图像合成方法，对影像进行归一化并去除云和天气缺陷；(2) 预测融合方法，降低对单张图像质量的依赖；(3) 标签迁移技术，将标注从单一来源迁移到多个图像源作为数据增强手段，提高数据效率。

Result: 图像合成方法使误差降低38.2%，预测融合方法使误差降低53.6%。多源标签迁移是性能提升最大的因素，在不同验证实验中误差降低28.1%至76.2%，同时将不同成像日期性能的标准差降低了13倍。

Conclusion: 所提出的数据融合框架能有效提升深度学习模型在木本植被分割中的鲁棒性和数据效率，多源标签迁移是最有效的策略，且显著提升了模型在不同成像条件下的性能稳定性。
...
TL;DR: 本文提出了一种基于文本到图像潜在扩散模型的零样本视频恢复与增强框架，通过双提示调优反转与采样将推理时间降至原来的1/3，并利用纹理感知视频令牌合并和参考自注意力机制显著提升时间一致性和恢复性能。


<details>
  <summary>Details</summary>
Motivation: 零样本图像恢复方法在图像任务上表现出色，但直接应用于视频恢复时会产生严重的时序闪烁问题，缺乏专门针对视频时间一致性的零样本恢复方案。

Method: 1) 提出双提示调优反转与采样（dual prompt tuning inversion and sampling），将推理时间缩减至约1/3；2) 提出纹理感知视频令牌合并（texture-aware video token merging），利用帧间时间相关性提升时间一致性；3) 提出参考自注意力（referenced self-attention）和参考令牌合并（referenced token merging）以支持图像参考。

Result: 实验结果表明，所提方法在恢复和增强时间一致视频方面优于现有方法，同时推理效率显著提升，性能和时间一致性均得到加强。

Conclusion: 通过多模态参考和多项技术贡献，该框架有效解决了零样本视频恢复中的时序闪烁问题，兼顾了推理效率和时间一致性，验证了将文本到图像潜在扩散模型泛化到视频领域的可行性。
...
TL;DR: 本文提出了一种基于双时相Sentinel-2影像的木质植被变化检测方法，通过引入损失缩放系数α来对齐模型目标与最终用户指标（Fβ分数），并利用图像增强与生成技术使模型能够零样本迁移到再生长检测和木质分割任务中，可显著提升精度和召回率。


<details>
  <summary>Details</summary>
Motivation: 深层学习模型在检测双时相遥感影像中木质植被变化时，由于损失函数定义与最终用户规格不一致，生成的产品可能无法满足用户需求。此外，深度学习模型依赖大规模标注数据集，而对于空间上稀有、语义模糊的事件（如再生长检测），获取充足数据非常困难。

Method: 使用7年澳大利亚新南威尔士州年际双时相Sentinel-2影像训练木质变化检测模型。引入损失缩放系数α，将训练目标转化为优化特定Fβ分数；提出输入影像增强与生成技术：（1）利用激活最大化技术（低α值保证稳定性）进行图像生成用于木质分割，（2）基于手工特征（马赛克清除斑块与人工树）的图像生成技术提供上下文基础，（3）通过生成伪后时相影像和前时相影像实现零样本再生长检测。

Result: 引入α后，精度提升1.85倍，召回率提升1.12倍。木质分割任务中，所提图像生成方法较先前研究减少总体误差高达18.2%。零样本再生长检测取得了0.845的F1分数。

Conclusion: 损失缩放系数α能有效对齐模型目标与用户指标，图像生成与增强技术可显著提升模型向再生长检测和木质分割任务的零样本迁移能力，为未来再生长检测工作奠定基础。
...
TL;DR: Video-FLAIR是一个训练框架，通过强化学习让视频多模态模型根据查询类型自适应选择感知、组合或深思推理模式，在提升准确率的同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有查询采用统一的推理策略，导致简单任务上不必要的计算开销和复杂任务上推理不足的问题。不同多模态查询需要不同类型的推理（感知、组合或深思），需要自适应的推理模式选择机制。

Method: Video-FLAIR是一种基于强化学习的训练框架。训练时模型在同一提示下生成三种推理模式（感知、组合、深思）的响应进行直接比较；通过一个复合奖励函数，基于正确性、grounding和计算成本来评估和比较这些响应，优先选择最有效的模式，同时惩罚无依据或不对齐的深思推理。由此获得自适应推理的监督信号，无需逐查询的人工标注。

Result: 在MathVista上比Qwen2.5-VL基线提升+5.4，在Video-Holmes上提升+4.8，在Video-MMMU上提升+4.8；同时将平均token使用量从始终深思基线的417降至95，大幅节省计算成本。

Conclusion: Video-FLAIR通过强化学习实现了视频多模态推理模式的动态选择，在准确率和计算效率之间取得了更好的权衡，证明了自适应推理策略在视频理解任务中的有效性。
...
TL;DR: 本文基于PRISMA指南对2015年1月至2025年1月间发表的97项OCR研究进行系统性文献综述，梳理了AI模型在OCR领域的演进历程，涵盖应用领域、数据类型、语言覆盖和挑战，并提出了自监督学习、多模态AI、AutoML等未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 尽管OCR技术在处理异构文本数据方面取得了显著进步，传统OCR模型在面对脚本变体、书写风格多样化和退化文档时仍存在局限。然而，目前缺乏对OCR技术进展的全面系统评估，因此有必要进行系统综述以追溯过去十年AI模型在OCR领域的演变。

Method: 遵循PRISMA（系统综述和荟萃分析优先报告条目）指南，对2015年1月至2025年1月间的97项精选研究进行系统性文献综述，分析OCR模型的演进、应用领域、数据类型、语言覆盖范围及面临的挑战，并对关键OCR模型的性能、优势和局限性进行深入剖析。

Result: 研究发现OCR技术已从处理结构化文本演进到非结构化文本、场景文本识别和多语言处理。仍存在的未解挑战包括：低资源语言的训练数据有限、手写文本高变异性、字符视觉相似性判别困难，以及实时OCR应用中的约束。

Conclusion: 提出了包括自监督学习、多模态AI、AutoML、AI辅助后处理、TinyML以及建立联合语料库用于脚本匹配等若干有前景的解决方案，旨在提升OCR准确性并解决实时工业应用中的挑战，为未来研究奠定基础。
...
TL;DR: NeuDonatello是一个新颖的神经表面重建框架，通过蒙特卡洛采样建模SDF空间变化的不确定性，并据此设计自适应正则化和不确定性感知的SDF到密度转换参数，从而在仅使用有姿态RGB图像的情况下实现高精度三维表面重建，达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 仅从RGB图像恢复精确几何形状仍具挑战性，因为无纹理区域、遮挡和场景固有歧义会引入不确定性。现有方法往往忽略这些不确定性，导致符号距离函数（SDF）估计不准确。

Method: 提出NeuDonatello框架：(1) 利用蒙特卡洛采样策略建模空间变化的不确定性；(2) 基于该不确定性开发自适应正则化，在RGB监督不可靠处选择性加强几何约束；(3) 引入不确定性感知的SDF到密度转换尺度参数，实现更精确的空间变化密度建模。

Result: 大量实验表明，NeuDonatello在仅使用有姿态RGB图像的情况下，在多种场景中实现了最先进的表面重建精度，且性能鲁棒。

Conclusion: 显式地建模和利用SDF不确定性可以显著提升神经表面重建的几何精度，为处理场景歧义和不确定性提供了一种有效范式。
...
TL;DR: 本文提出HUG-VIS，一个统一的、以人为中心的视觉智能基准数据集，包含8,400段由30名专业演员录制的坐姿半身视频，覆盖280种情绪-动作-提示组合，同步提供视频、音频、文本和alpha遮罩，用于统一评估和推动四个代表性任务（情绪识别、视频生成、语音克隆、视频抠像）的研究进展。


<details>
  <summary>Details</summary>
Motivation: 现有的以人为中心的视觉智能研究资源是任务特定的，每个任务有其单独的数据集和标注，缺乏一个共享的、统一的基础设施来协调理解与生成类任务，这限制了多模态信号的利用和更广泛的研究推进。

Method: HUG-VIS包含8,400段受控普通话工作室协议下录制的坐姿半身视频，30名专业演员完成280种情绪-动作-提示分配，提供同步的视频、音频、文本和alpha遮罩。采用统一的零样本评估协议，使用自动指标、基于标准的主观评分（MOS）和多项跨任务分析，评估多种开源和闭源模型在四个任务上的表现。

Result: 结果发现：(i)当前情绪识别主要由语言内容主导，纯视觉情绪识别最弱；(ii)在视频生成和语音克隆中，自动指标与人类判断总体一致但在排名上存在差异，需要联合报告；(iii)运动状态下的边界保真度是人体抠像的主要障碍；(iv)任务难度随情绪、模型和指标而变化，并存在显著的跨任务相关性。

Conclusion: HUG-VIS填补了以人为中心的视觉智能缺乏统一基准的空白，为理解与生成四类任务提供共享的多模态基础，其发现可指导未来研究方向的改进，如增强纯视觉情绪感知、改进运动边界抠像质量以及结合自动指标与人类判断的综合评估。
...
TL;DR: 提出FAN-LoRA，一种面向医学影像分割的频域解耦参数高效微调架构，通过B样条驱动的低通分支和高通傅里叶分支显式分离低频结构对齐与高频纹理补偿，解决SAM在医学影像跨模态/跨中心分布偏移下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: SAM等视觉基础模型在自然图像分割中表现优异，但直接迁移到医学影像面临跨模态和跨中心等严重域间隙。现有PEFT方法在严重分布偏移下性能显著退化，根本原因是异构频率分量在共享低秩子空间中被隐式纠缠，导致结构对齐不佳和边界模糊。

Method: 提出FAN-LoRA架构，显式解耦优化空间：1）B样条驱动的低通分支负责全局结构对齐；2）离散傅里叶高通分支负责局部纹理补偿。两个分支协同工作以分离低频和高频优化目标。

Result: 在三个跨模态和跨中心基准上，FAN-LoRA持续优于SOTA PEFT基线，相比最强竞争者实现平均Dice分数的一致提升和边界误差的显著降低，同时保持紧凑模块尺寸和计算效率。

Conclusion: 通过频域解耦机制有效缓解了PEFT方法在分布偏移下的性能瓶颈，为医学影像分割中的基础模型适配提供了更鲁棒的解决方案。
...
TL;DR: 本文提出视觉信息引导采样器（VIG-Sampler），在扩散多模态大语言模型（dMLLMs）的逐步去掩码解码过程中，根据token对图像token的注意力来优先选择解码token，并约束候选token的图像注意力分布与已选token的差异，从而提升信息增益。在7个标题生成和VQA基准上、基于3个开源dMLLMs的实验中，VIG-Sampler平均优于Info-Gain Sampler 19.3个CIDEr点，且在COCO Caption上仅用一半解码步数即超越后者。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型的解码质量高度依赖每一步选择的token，因为已选token构成后续预测的上下文。现有策略要么基于确定性度量（偏向训练数据中高频token），要么基于对后续预测的影响排序但不显式考虑输入图像，缺乏对视觉信息的显式利用。

Method: 提出VIG-Sampler，核心思想是：(1) 根据token对图像token的注意力（cross-attention）优先选择视觉信息丰富的token；(2) 引入约束惩罚，使候选token的图像注意力分布与已选token的图像注意力分布尽量不同，从而最大化解码子集的整体信息增益。

Result: 在7个标题生成和VQA基准上、3个开源dMLLMs上验证有效性：平均超越Info-Gain Sampler 19.3个CIDEr点（标题生成基准），在COCO Caption上以一半解码步数超越后者。

Conclusion: 显式利用图像注意力信号来引导解码token的选择，能有效提升dMLLMs生成质量，同时通过信息增益约束减少冗余选择，甚至能减少所需解码步数，证实了视觉信息在采样策略中的关键作用。
...
TL;DR: 提出DPA-I2P，通过深度引导的投影对齐方法解决图像到点云配准中的跨模态对应学习难题，在KITTI和nuScenes数据集上显著优于现有隐式基线方法。


<details>
  <summary>Details</summary>
Motivation: 图像到点云配准是自动驾驶和大规模室外定位中的基础任务，但图像与稀疏LiDAR点云之间存在固有的模态差异，导致可靠的跨模态对应学习仍然具有挑战性。

Method: 提出Depth-Guided Projective Alignment (DPA-I2P)框架，包含三个核心模块：1) Ray-Conditioned Metric Depth Encoding (RMDE)用于结构化深度编码；2) Projection-Consistent Vision Lifting (PVL)以几何感知方式利用深度和视觉线索；3) Cross-Modal Query Pruning (CQP)在早期细化阶段抑制不可靠的查询以提升匹配稳定性。

Result: 在KITTI数据集上，DPA-I2P相比最强隐式基线将RTE降低45.0%，RRE降低55.6%；在nuScenes数据集上也优于所有评估的基线方法，展现出更好的跨场景迁移能力。

Conclusion: 深度引导的投影对齐策略能够有效弥补跨模态差异，提升图像到点云配准的精度和稳定性，并且在不同驾驶场景下具有良好的泛化性。
...
TL;DR: 本文提出FU-Mamba，一种用于口内扫描图像分割的框架，通过动态扫描和频域增强解决视觉状态空间模型中的语义空间连续性问题及成像条件不利导致的频域分布失衡问题，在牙科分割数据集上mIoU提升1.1%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉状态空间模型依赖人工设计的扫描顺序将图像块展平成序列，破坏了语义空间连续性，阻碍前景关键区域的特征提取；同时口内扫描过程中的光照不均、表面反光和噪声会削弱高频细节、增强低频成分，影响边界精确定位。

Method: FU-Mamba包含两个核心模块：(1) 动态Mamba块（DMB），通过可训练的偏移预测网络自适应学习采样偏移，并执行灵活的bilinear插值，实现内容感知扫描以保持空间连贯性；(2) 频域增强块，通过小波引导分解和频谱池化平衡频谱成分，提升在不利成像条件下的鲁棒性。

Result: 在牙科分割数据集上，FU-Mamba的平均交并比（mIoU）较基线提升1.1%，验证了其分割精度的显著提升。

Conclusion: FU-Mamba通过将动态扫描策略与频域增强机制引入SSM架构，有效解决了语义空间断裂和频域失衡两大挑战，为数字牙科中的口内扫描图像分割提供了更准确且鲁棒的解决方案。
...
TL;DR: 本文提出Text-to-Seed (T2S)框架，将开放词汇语义分割重新表述为"文本引导的种子点定位+基于种子的区域扩展"两步流程，利用Stable Diffusion的文本-区域对应关系生成种子点，再作为SAM的点提示生成完整目标掩码，无需训练即可在标准基准上取得良好性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇语义分割方法依赖SAM来细化其他模型产生的粗掩码，但当初始掩码不准确时这种策略不可靠。作者认为更可靠的方式是利用SAM作为区域扩展模块，以准确的目标点（种子）为引导，而不是依赖不准确的粗掩码。

Method: T2S框架包含两个步骤：(1) 利用Stable Diffusion的文本-区域对应关系生成基于注意力的种子点，定位目标类别对应的区域；(2) 将这些稀疏种子点作为SAM的点提示（point prompts），生成完整的目标对象掩码。整个框架无需任务特定训练或额外标注。

Result: 在标准开放词汇语义分割基准上取得了强性能，证明了将语义定位与种子驱动的空间分割相结合的有效性。

Conclusion: 通过将SAM从"粗掩码修正器"重新定位为"种子引导的区域扩展器"，并利用Stable Diffusion的语义定位能力，T2S提供了一种无需训练、无需额外标注的可靠OVSS新范式。
...
TL;DR: 本文提出组合步态检索（CoGR）新任务，通过参考序列和自然语言修改指令检索目标步态序列。作者利用大视觉语言模型自动构建了首个步态-语言数据集（Language-Augmented CCPG 和 Language-Augmented CASIA-B），并提出 ComposeGait 身份锚定组合框架，通过 Part-aware Identity Adapter 聚合身份信息并注入共享 Q-Former 两个分支以保持身份一致性，在两项基准上分别达到 72.38% 和 83.61% 的 R@1。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法局限于刚性视觉匹配，忽略了自然语言指令在交互式检索中的潜力。组合检索中常见的身份漂移问题（模型遵循指令却返回错误的人）尚未在步态领域被解决，且缺乏步态-语言组合检索数据集，使得该任务无法开展。

Method: 首先设计基于大视觉语言模型（VLMs）的自动化标注流水线，构建 Language-Augmented CCPG 和 Language-Augmented CASIA-B 两个步态-语言数据集。随后提出 ComposeGait 框架：Part-aware Identity Adapter（PIA）将多帧、部位感知的身份证据聚合为样本特定 ID token，并注入共享 Q-Former 的两个分支以保持身份，同时将 ID-token 输出从最终检索嵌入中排除；通过联合身份损失和任务自适应组合检索目标端到端优化嵌入空间。

Result: 在 Language-Augmented CCPG 和 Language-Augmented CASIA-B 两个基准上，ComposeGait 分别达到 72.38% 和 83.61% 的 R@1，超越所有对比方法，成为 CoGR 任务的强基线。

Conclusion: ComposeGait 证明了在组合步态检索中显式锚定身份信息能有效防止身份漂移，为步态识别与自然语言交互的结合开辟了新方向；数据集和代码将公开以促进后续研究。
...
TL;DR: 本文提出了一种轻量级、无需训练的框架，基于时空聚类实现事件相机移动目标发现。核心包括线性复杂度的时空概率事件滤波器(SPEF)用于去噪，以及基于事件Morton编码聚类(EMCC)模块用于高效分组移动目标。在E-MLB去噪基准、FRED和eTraM目标发现基准上均取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 事件相机以微秒级分辨率捕获像素级强度变化，产生高度稀疏的异步数据流。在延迟敏感环境中进行视觉感知时，需要一种轻量级且无需训练的框架来发现移动物体，以克服现有方法依赖昂贵距离矩阵计算或离线训练的限制。

Method: 框架包含两个核心模块：(1) 时空概率事件滤波器(SPEF)——引入自适应事件接受阈值，在线性时间内区分显著运动结构与背景噪声；(2) 事件Morton编码聚类(EMCC)——利用Morton编码（空间填充曲线）绕过昂贵的距离矩阵计算，高效地对事件进行分组，实现移动目标的非监督发现。

Result: 在E-MLB数据集上，SPEF的降噪性能在经典滤波方法中最佳，且与基于学习的方法竞争力相当但无需离线训练。在目标发现任务上，EMCC在FRED和eTraM数据集上取得最高总体精度和最低执行时间，大幅超越基于密度的聚类基线。
...
TL;DR: 提出AGW-PBR方法用于脑MRI超分辨率，通过组织混合熵权重强调组织过渡区域（受部分体积效应影响），在T2加权IXI图像和多尺度（2x/4x/6x）上取得更优的全图像重建和区域保真度。


<details>
  <summary>Details</summary>
Motivation: 传统全图像目标函数在脑MRI超分辨率中会低估受部分体积效应影响的组织过渡区域，因为这些区域仅占图像的一小部分；此外，二值边界无法捕捉体素内脑脊液、灰质和白质的连续混合。

Method: AGW-PBR结合了仅低分辨率的重建骨干网络与训练时目标函数。骨干网络整合LR导出的Sobel引导、软潜基分配和有界网格锚定残差变形；训练时利用来自已配准T1/T2/PD IXI图像的组织分数转换为组织混合熵，定义均值归一化的重建权重（仅在训练时使用，推理仅需LR图像）。

Result: 在T2加权IXI图像上以2x/4x/6x评估，使用三种种子和受试者级配对分析。4x时用测试专用SynthSeg掩膜独立评估组织界面及非界面区域的重建质量。AGW-PBR改善了全图像重建和4x的区域保真度；无PVE监督的骨干在fastMRI 4x上保持强性能。

Conclusion: 组织混合熵加权有助于实现部分体积感知的脑MRI超分辨率，支持将组织混合熵权重作为PVE敏感区域的训练策略。
...
TL;DR: 本文提出层次化通道堆叠（HCS）框架，将CNN中间激活转化为跨三个骨干阶段的60维结构化表示，用于AI生成图像检测，在GAN和扩散生成器基准上达到86.7%准确率与宏F1，并揭示不同伪造类型在阶段贡献上的差异。


<details>
  <summary>Details</summary>
Motivation: 现有合成图像检测器虽预测准确，但缺乏对决策过程的解释性，难以理解模型如何在不同表示层级上整合证据。

Method: HCS通过三个逐渐加深的骨干阶段将CNN中间激活转换为60维结构化表示；使用通道级Level-1分类器与Level-2聚合器生成图像级预测，保留显式层次结构用于分析。

Result: 在跨越GAN与扩散生成器的基准上，HCS在保留测试集上达到86.7%准确率和86.7%宏F1；阶段消融实验证实三阶段完整系统优于单阶段和两阶段变体；阶段级贡献分析显示伪造GAN与伪造扩散图像具有不同的阶段贡献特征。

Conclusion: HCS不仅是紧凑的检测器，更是一个用于研究合成图像检测器如何跨表示层级组装证据的结构化分析框架。
...
TL;DR: 提出CoGeo-GS，一个用于3D场景中可控多物体移除的概念驱动框架，通过概念感知语义标签和几何感知补全管线实现稳定、多视图一致的物体移除。


<details>
  <summary>Details</summary>
Motivation: 多物体移除面临严重遮挡、语义纠缠以及难以保持几何和多视图一致性的挑战。现有3DGS方法在单物体编辑上表现良好，但在多物体场景中扩展性差，需要重复优化且移除区域几何不稳定。

Method: CoGeo-GS包含两个核心组件：(1) 概念驱动的语义标签机制——为高斯分配概念感知语义标签，实现灵活物体选择并减少前景物体与背景结构之间的干扰；(2) 几何感知补全管线——结合单目深度先验、扩散模型精炼和边界对齐混合来恢复移除区域的几何；此外采用几何正则化精炼策略稳定重建并保持多视图一致性。

Result: 实验表明CoGeo-GS在视觉质量和重建保真度上均优于现有方法。

Conclusion: CoGeo-GS通过在单一优化阶段内实现概念驱动选择与几何感知补全，有效解决了多物体移除中的遮挡、语义纠缠和几何稳定性问题，为3D场景编辑提供了一种更可靠且可扩展的方案。
...
TL;DR: 本文提出PailitaoGR，一种基于"潜在图像思考"（Latent Think-with-Images）的生成式图像检索方法，通过目标聚焦感知和选择性辅助证据利用两个机制，实现"无需裁剪的缩放"和"无需OCR的阅读"，在真实在线图像搜索日志数据集上平均优于基线13.8%。


<details>
  <summary>Details</summary>
Motivation: 生成式检索在基于产品语义标识（SIDs）的检索中表现出色，但将其扩展到图像搜索面临挑战：真实查询图像包含复杂多样信息（搜索目标、有用辅助证据和无关视觉内容），模型需要识别并聚焦于搜索目标，同时选择性利用辅助证据，而现有方法难以同时处理这些需求。

Method: 提出PailitaoGR框架，包含两大核心机制：(1)目标聚焦感知机制，通过目标增强器（target Enhancer）结合基于on-policy蒸馏和注意力引导损失的训练策略，增强搜索目标的视觉token；(2)选择性辅助证据利用机制，通过辅助增强器结合in-capacity增量对比蒸馏策略，实现对辅助证据的选择性利用。训练和验证集从真实在线图像搜索日志中采样构建。

Result: 在真实在线图像搜索日志构建的训练和验证集上，PailitaoGR相比现有基线平均提升13.8%，验证了方法的有效性。

Conclusion: PailitaoGR成功将生成式检索范式适配到图像搜索场景，通过"潜在思考"的方式在模型内部实现目标聚焦与辅助证据的选择性利用，实现了"无需裁剪的缩放"和"无需OCR的阅读"效果，为生成式图像检索提供了有效方案。
...
TL;DR: 本文提出HullWake框架，通过"船体优先、尾流其次"的策略解决海上船舶检测中因依赖尾流而导致的问题，并引入面向尾流的评估协议和约1万张图像的Curated-Wake数据集。


<details>
  <summary>Details</summary>
Motivation: 海上船舶检测器常面临船体小、低对比度或模糊的场景，而尾流更长更易检测，导致检测器过度依赖尾流：可能漏检尾流弱或无尾流的慢速/静止船舶，或在类似尾流的水面杂波上产生误检。

Method: HullWake框架包含：（1）将基于提议的船体证据与方向性尾流上下文分离；（2）通过双向提议锚定通道提取尾流线索；（3）通过尾流响应监督、尾流衰减一致性、仅尾流置信度抑制和船体-尾流去相关来抑制尾流主导的预测。同时提出面向尾流的评估协议，涵盖弱/无尾流船舶、类尾流硬负样本、最差组AP和尾流衰减后的置信度下降。

Result: 在Curated-Wake数据集（约10,000张图像，来自Ships/Vessels in Aerial Images、SMD基准和SeaDronesSee，并新增检测级和分割级尾流标注）上，相比仅框检测器和掩码监督的分割基线，HullWake在整体AP、弱/无尾流鲁棒性、类尾流误检、最差组AP以及尾流衰减后的置信度稳定性方面均取得改进。

Conclusion: HullWake通过解耦船体与尾流证据并抑制尾流主导预测，有效解决了船舶检测中的尾流依赖问题，显著提升了弱/无尾流场景下的鲁棒性并减少了类尾流误检。
...
TL;DR: 提出RECAP-Forcing，一种按外观新颖性而非时间顺序组织KV缓存记忆的无训练推理方法，用于长自回归视频生成中的长期一致性维护。


<details>
  <summary>Details</summary>
Motivation: 长自回归视频生成面临有限注意力窗口下的记忆挑战：模型必须在不断增长的历史中决定保留哪些信息。现有方法按时间组织记忆，保留近期帧而压缩或丢弃旧帧，无法保证长视频中主体、物体、场景的身份一致性。作者提出按外观新颖性组织记忆的理念，使记忆规模与新增内容量而非视频长度成正比。

Method: RECAP-Forcing包含两种统一机制：(1) 注意力汇（attention sink）：在视频开头所有可见内容均为新颖时保留初始场景；(2) 基于光流的新颖性库（novelty bank）：随视频演进选择性保留新出现的可见内容（如进入的主体、遮挡解除区域、新场景）。整个方法为无训练推理方法，不引入额外可学习参数，通过按外观索引KV缓存实现长程一致性。

Result: 实验表明RECAP-Forcing在多个强基线模型上一致地提升了视觉质量和语义保真度，并优于现有的记忆方法。

Conclusion: 按外观新颖性索引的记忆结构是有效的替代方案，使长程一致性成为记忆结构本身的显式属性，而非依赖压缩或丢弃策略。
...
TL;DR: 提出Echo-GRPO框架，通过将教师模型的推理轨迹改写为与学生策略自身"idiolect"（个性化词汇与表达方式）一致的版本，解决GRPO强化学习中学生模型无法学习超出自身推理能力的问题，并在视频推理蒸馏任务中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: GRPO的on-policy特性将模型限制在其已能产生的推理技能范围内，限制了其学习更高级能力。先前工作注入强教师模型的特权推理轨迹指导训练，但这些轨迹与学生策略存在分布不匹配（out-of-distribution），导致梯度裁剪作用于语义关键的推理token上，最终模型只学到了正确答案却未学到支撑答案的推理过程。

Method: 提出Echo-GRPO框架，核心思想是"让模型用自己说话的方式推理"。具体做法：不从教师模型模仿低概率的特权轨迹，而是通过Dual-Reference Decoding将教师轨迹改写为学生策略自身的idiolect（个性化词汇和表达模式），同时保持语义不变。该框架实例化为VideoEcho-R1用于视频推理蒸馏。

Result: 在三个多模态LLM骨干和五个基准上取得一致的性能提升；同时证明idiolect改写是一个即插即用模块，能同时提升RL和SFT框架的推理蒸馏效果，表明策略对齐的监督信号适用范围超越GRPO。

Conclusion: 策略对齐的监督（即按照学生自身表达习惯改写外部推理轨迹）是提升推理蒸馏的关键，且该思想不仅适用于GRPO强化学习框架，也能推广到监督微调框架中。
...
TL;DR: 本文在资源受限条件下，研究SimSiam和SimCLR两种对比自监督学习框架用于眼底图像视网膜疾病分类，发现针对视网膜图像特性定制增强策略能显著提升性能，轻量级SSL框架即使在有限数据和算力下也能学到可迁移表示，减少对大规模标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 医学图像标注数据稀缺且获取成本高昂，监督学习方法依赖大量人工标注，在医学领域难以大规模应用。对比自监督学习（SSL）无需标签即可学习有用表示，为缓解标注瓶颈提供了可行方案，但尚缺乏在资源约束下针对眼底图像的系统性研究。

Method: 研究两种SSL框架（SimSiam和SimCLR），在资源受限设置下使用小批量大小训练，并引入针对视网膜图像特性的专用增强技术。通过线性评估和微调两种方式，在多元疾病分类和糖尿病视网膜病变分级两个下游任务上评估所学表示的质量。

Result: 实验表明，根据视网膜图像特征定制增强策略对提升性能至关重要。即使在资源受限条件下，轻量级SSL框架仍能学习到具有可迁移性的表示，在减少对大规模标注数据依赖的同时取得有竞争力的结果。

Conclusion: 针对视网膜图像的定制化增强策略是自监督表示学习的关键因素；轻量级SSL框架在数据与算力受限的医学场景下是可行的，能够有效降低对大规模标注数据的依赖并实现竞争性下游任务性能。
...
TL;DR: 本文提出AesCanvas统一基准套件，包含CritiqueCanvas（51.9万条指令-响应对，覆盖54,300张图像的多维度长篇美学批评）和ContextCanvas（301个专家审阅场景，评估图像在特定用途、受众和文化背景下的情境适宜性），系统评估了闭源前沿、开源通用和美学专用MLLM，发现批评生成与情境敏感判断之间存在显著分离，美学专用模型在情境适宜性上明显落后于强通用模型。


<details>
  <summary>Details</summary>
Motivation: 现有美学评估基准主要考察内在视觉质量或固定领域标准，未回答一个有吸引力的图像是否适合特定用途、受众、文化背景或领域规范这一问题，缺乏对情境化美学适宜性的系统评估。

Method: 构建AesCanvas双组件基准：CritiqueCanvas包含54,300张图像和519,136条指令-响应对，支持摄影、绘画和虚拟图像的长篇多维度批评；ContextCanvas包含301个专家审阅的现实使用场景，评估情境美学适宜性。在统一协议下评测闭源前沿、开源通用和美学专用MLLM。

Result: 批评生成与情境判断能力出现明显分离：基于参考的词汇和语义指标只能部分捕捉批评质量；美学专用模型在部分批评指标上有竞争力，但在ContextCanvas上大幅落后于强通用MLLM；美学专门化不能可靠迁移到情境适宜性判断；模型决策可能无法追踪或锚定关键的情境视觉线索。

Conclusion: 文化情境化、证据支撑的适宜性应作为美学建模的一个独立目标，现有美学专用模型在此方面存在显著不足。
...
TL;DR: 本文提出组合布局理解任务及CoDeLayout数据集（约2万张真实多图层布局VQA样本），并提出了MASON后训练范式，通过多模态对齐（MA）和结构感知（SP）分别缓解语义漂移和层级结构歧义问题，使Qwen2.5-VL 7B达到91.66%准确率，超越GPT-o3的79.68%，且仅用30%训练数据即超越全量直接微调。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）在理解由独立元素组成的原子布局方面表现良好，但在处理需要推理视觉纠缠元素、层级多图层结构的组合布局时表现不足。文档分析、UI创建和图形设计等实际应用广泛依赖组合布局理解，因此需要新的任务定义、数据集和解决方案。

Method: 本文提出：(1) 新任务组合布局理解及CoDeLayout VQA数据集，包含约2万张真实多图层布局，标注有组合元素对和设计意图；(2) MASON后训练范式，包含多模态对齐（MA）模块，通过将元数据定义的元素锚定到其视觉对应物来缓解语义漂移，以及结构感知（SP）模块，建模图层感知的元素间空间关系以提升层级理解。

Result: 实验表明现有VLM存在显著差距：最强基线GPT-o3仅达79.68%准确率，而采用MASON的Qwen2.5-VL 7B达到91.66%。MASON仅使用30%训练数据即超越全量直接微调，并且随数据量增加表现出更好的扩展性。

Conclusion: 组合布局理解是一个具有挑战性的新任务，现有VLM存在明显性能差距。MASON后训练范式有效解决了语义漂移和结构歧义两大挑战，在数据效率方面显著优于直接微调，为布局理解研究提供了新方向和高价值的数据与训练范式。
...
TL;DR: 本文提出sLoTh，一个面向预训练脉冲事件视觉Transformer的参数高效持续学习框架，通过冻结骨干网络并仅更新可扩展的低秩注意力（seLoRA）和共享神经元阈值，在不使用回放缓冲的情况下实现类增量与在线持续学习，同时比传统稠密视觉Transformer降低约6.5倍能耗。


<details>
  <summary>Details</summary>
Motivation: 机器人与边缘智能系统面临动态数据流，需要在严格内存和能耗约束下持续适应并保留旧知识。尽管参数高效微调在视觉Transformer持续学习中展现出潜力，但传统稠密架构计算开销大，难以满足实际部署需求。稀疏事件视觉Transformer虽提供能效优势，但其持续学习能力尚未被充分探索。

Method: sLoTh冻结骨干网络，将可塑性限制在两部分：可扩展高效低秩注意力更新（seLoRA）和共享神经元阈值调制。该方法仅更新不到1%的模型参数，无需回放缓冲区即可实现适应。

Result: 在CIFAR-100、Tiny-ImageNet、ImageNet-100和ImageNet-R上，涵盖最多100个任务的实验表明，sLoTh在无回放的类增量学习和在线持续学习中取得了具有竞争力的性能，同时相比传统稠密视觉Transformer实现了约6.5倍的能耗降低。

Conclusion: sLoTh证明了预训练稀疏事件视觉Transformer在持续学习中不仅可行且高效，通过极少量参数更新即可实现竞争性性能，同时大幅降低能耗，为资源受限场景下的自适应学习提供了有效方案。
...
TL;DR: 提出UniGeo，一个统一的多模态大语言模型（MLLM），用于文本引导的无人机地理定位任务，通过联合支持地理语义理解、跨视角语义生成和候选级验证，在多检索骨干上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导无人机地理定位方法将任务简化为开放式文本查询与候选图像的直接全局匹配，但在查询不完整和候选高度相似的情况下，全局跨模态匹配不足以实现可靠的细粒度定位。

Method: UniGeo构建于共享视觉-语言框架之上，包含三个核心能力：(1) 通过地理语义学习建立局部场景元素、空间关系与语言描述之间的稳定对应；(2) 通过跨视角生成建模无人机视角与卫星视角之间的语义映射；(3) 一个即插即用的验证模块对高度混淆的候选进行细粒度判别。同时引入多阶段训练策略，逐步学习地理语义理解、跨视角生成与候选验证。

Result: 在GeoText-1652基准上，UniGeo将R@10提升13.59个百分点，mAP提升2.83个百分点，并在多个检索骨干网络上表现出一致的性能提升。

Conclusion: UniGeo通过统一的多模态大语言模型框架，有效解决了文本引导无人机地理定位中全局匹配不足的问题，验证了地理语义理解、跨视角生成与候选级验证相结合对细粒度定位的有效性。
...
TL;DR: 提出GeoMAD，一个统一的多视角多类别异常检测框架，通过跨视图可变形融合模块（CDFM）和分布性视图对齐（DVA）分别解决几何对应不足和分布不一致问题，在2D特征空间中实现高效几何感知融合，无需相机标定、体素构建或类别特定3D监督。


<details>
  <summary>Details</summary>
Motivation: 多视角异常检测（MvAD）的关键挑战是在融合多相机视角信息时兼具几何感知能力与多类别工业场景的可扩展性。现有方法存在两个极端：基于体素的融合虽提供显式几何对齐，但需要昂贵的3D构建和类别特定假设；轻量级基于patch的融合虽高效，但依赖离散候选匹配，缺乏连续的跨视图对应关系。

Method: 提出GeoMAD框架，包含两个核心模块：（1）跨视图可变形融合模块（CDFM），在2D特征图上直接学习内容自适应、视图对特定的采样偏移，并通过多尺度窗口金字塔与图像全局参考采样进行层次化组织，实现无需相机标定、体素构建或类别特定3D监督的跨视图对应；（2）分布性视图对齐（DVA），一种自监督跨视图正则化损失，将每个视图的瓶颈分布对齐到实例级的视图中心目标，在无需像素级对应的前提下强制全局一致性。

Result: 在Real-IAD和MANTA-Tiny数据集上的大量实验表明，GeoMAD在统一的多视图异常检测任务中取得了强检测与定位性能。

Conclusion: CDFM和DVA分别桥接局部几何对应与全局分布一致性，在保持2D特征空间学习高效性的同时，提供几何感知且分布一致的融合，为多视图多类别异常检测提供统一解决方案。
...
TL;DR: 该论文将户外LiDAR语义场景补全（SSC）重新定义为生成式语义场景补全（GSSC），通过统一的离散扩散框架解决三个关键问题：合成配对稀疏-稠密数据以缓解极端类别不平衡、生成语义场景补全、以及单步流匹配精化已有补全结果。在SemanticKITTI隐藏测试集上达到38.8% mIoU，是该榜单上无因果约束限制下的最佳结果。


<details>
  <summary>Details</summary>
Motivation: 户外LiDAR语义场景补全面临两个核心挑战：扫描仅观测目标体积的1%导致极端稀疏性，以及类别不平衡超过7000倍。现有方法受限于数据稀缺和长尾分布，性能受限。作者提出将SSC重新构思为生成式方案，从数据合成、生成建模和精化三个层面系统性应对这些挑战。

Method: 提出统一的离散扩散框架GSSC，包含三个角色：(1) PS³用于配对稀疏-稠密场景合成，生成匹配的LiDAR观测与其稠密语义补全，构建PS³-SemanticKITTI训练语料；(2) SGSC利用多项离散扩散从噪声生成场景，以鸟瞰语义图和稀疏3D特征流为条件；(3) S²D²作为结构化源离散扩散，通过单步流匹配精化已有补全结果，无需基础模型重训练或测试时适配。

Result: S²D²在无需基础模型重训练的情况下提升了SGSC自身输出及所有外部SSC基线的mIoU。在最强基线上，单步且无测试时增强达到38.8% mIoU（SemanticKITTI隐藏测试集），为因果、单扫描、单样本条件下的最佳发布成绩，较此前最佳结果提升+2.1个百分点；四步校正配合八视图测试时增强达到39.2%。

Conclusion: 生成式方案为LiDAR语义场景补全提供了一条有效路径：通过在数据源头解决长尾分布问题、以生成建模捕捉场景先验、并用统一框架实现精化，显著超越了传统判别式方法。该框架的通用精化能力使其可作为即插即用模块提升任意SSC基线的性能。
...
TL;DR: 本文提出G2D框架，一种无需训练的零样本分类方法，通过生成式视觉语言模型（VLM）对CLIP检索出的候选标签进行细粒度验证，将判别式模型的广域检索能力与生成式模型的细粒度视觉推理能力互补结合，在八个基准上实现68.85%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 判别式模型（如CLIP）和生成式模型在零样本分类中存在互补性的失败模式：当CLIP的top-1预测错误时，正确标签往往仍在其top-K候选列表中，因此关键挑战是消歧而非召回；而独立的生成式模型则受限于大规模标签空间和不受约束的输出。这一互补性促使将宽泛的候选检索与细粒度的、以图像为锚点的验证相分离。

Method: G2D是一个无需训练的框架，核心流程为：先用CLIP进行候选检索，再由生成式VLM对候选标签逐一对图像进行验证。具体设计包括：固定置信度路由（将生成式推理聚焦于不确定样本）、熵自适应候选规模（根据不确定性动态调整候选数量）、以及trie约束解码（确保每个输入在测试时输出唯一合法结果）。候选名称和CLIP概率构成结构化先验，用于区分视觉相似的类别。

Result: 在八个基准上，G2D达到68.85%的平均准确率，显著优于CLIP（59.35%）和独立VLM（63.11%）。在七种生成器配置下，候选集验证相比基线平均提升1.08至27.42个百分点。G2D还可迁移到DCLIP、WaffleCLIP和CuPL等变体上。

Conclusion: G2D验证了判别式候选生成与生成式视觉推理之间的实用接口范式，通过结构化先验和工程化策略（路由、自适应尺寸、约束解码）有效解决了判别式模型的消歧难题和生成式模型的无约束输出问题，为零样本分类提供了有力的补充方案。
...
TL;DR: 提出范式转变，将玻璃表面检测从2D外观线索转向3D视觉几何，通过蒸馏VGGT的3D先验并引入频率自注意力与几何接地模块，在七个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 玻璃表面的透明性和反射性使检测具有挑战性；现有方法依赖2D外观线索，在几何模糊场景中容易失效，缺乏对玻璃物理存在的显式建模。

Method: 先从视觉几何变压器VGGT中蒸馏丰富的3D先验并生成玻璃感知的3D表示；再利用多任务学习框架，包含两个核心模块：频率自注意力模块（FSAM）识别玻璃特定频谱特征用于定位，几何接地模块（GeGB）将2D特征选择性地接地到3D几何中用于分割。

Result: 在七个标准GSD基准上达到SOTA性能，能泛化到视频/多模态数据，并显著改善玻璃场景的重建效果。

Conclusion: 将玻璃表面检测奠基在3D视觉几何上能显式建模玻璃的物理存在，优于单纯依赖2D外观线索的方法，为该领域提供了新的研究范式。
...
TL;DR: 本文研究从DINOv2教师（Vision Transformer）到紧凑双向视觉状态空间模型（LVSSM）学生的跨架构知识蒸馏，用于茶叶叶片病害分类。通过修复两个训练稳定性问题（大的patch-embedding卷积和切断残差路径的融合层），采用渐进式卷积stem和门控双向选择性扫描块，使4.45M参数的轻量学生在三颗种子上将测试准确率从92.32±2.14%提升到95.41±1.17%，相比22M教师仅减少98.3%的精度同时降低5倍参数。消融实验表明中间特征对齐损失反而降低性能，简单logit级蒸馏效果最佳。


<details>
  <summary>Details</summary>
Motivation: 自动茶叶病害分类支持精准农业，但在严格计算预算下在边缘设备部署准确模型仍具挑战。自监督视觉基础模型（如DINOv2）特征强但体积过大不适合现场部署，而在小规模农业数据集上从头训练的轻量模型容易欠拟合。跨架构知识蒸馏是一个尚未充分探索的方向，因为Transformer和状态空间模型使用根本不同的token混合机制。

Method: 从微调后的DINOv2 ViT教师向紧凑的双向视觉状态空间模型（LVSSM）学生进行知识蒸馏。识别并修复了两个导致SSM学生从头训练不稳定的问题：单个过大的patch-embedding卷积和切断残差路径的融合层。采用渐进式卷积stem和门控双向选择性扫描块替代。使用温度缩放logit蒸馏在三颗种子上训练4.45M参数的学生模型。

Result: logit蒸馏将测试准确率从92.32±2.14%提升至95.41±1.17%（最佳单次运行96.20%，宏F1为94.45%），平均提升+3.09个百分点。学生参数比22M教师少5.0倍，同时保留98.3%的教师精度。消融显示中间特征对齐损失降低准确性，简单logit级KD是最强配置；公平从头比较显示提升仅对低于教师水平的学生有效。

Conclusion: 该研究证明跨架构知识蒸馏（从ViT到SSM）在资源受限的农业边缘部署场景中有效，通过修复SSM架构的训练稳定性问题并采用简单logit蒸馏，能显著提升轻量学生模型的性能。作者报告了逐类指标、混淆矩阵、bootstrap置信区间和FLOPs/延迟测量，但承认局限性包括单一数据集和简化的非官方SSM实现。
...
TL;DR: 本文提出Ring Forcing，一种自回归视频扩散框架，通过环形结构化训练策略强制模型从远距离历史中检索信息，同时引入压缩与时间步组合策略以及稀疏RoPE机制，解决长时视频生成中物体恒存性与记忆容量两大瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 长时视频生成存在关键瓶颈：现有模型缺乏鲁棒的长期记忆。该问题体现在两个层面——物体恒存性（物体重新出现时无法精确还原其外观）和记忆容量（无法处理超长上下文并利用远距离历史信息）。二者缺一不可：仅有物体恒存性而无足够的上下文处理能力会限制时间跨度，仅有长上下文而无恒存性则无法保持物体身份一致性。

Method: 提出Ring Forcing框架，包含三个核心组件：(1) 环形结构化训练策略，强制模型从远距离历史中检索信息，在严格历史遵循与生成多样性之间取得平衡；(2) 压缩与时间步组合策略，在固定序列长度约束下扩展有效历史跨度至分钟级时长，实现对全部历史的综合感受野；(3) 稀疏RoPE机制，支持灵活可扩展的记忆适配，同时充分利用预训练先验。

Result: 大量实验表明，Ring Forcing在分钟级连贯性和物体恒存性方面均取得优异表现，显著优于现有最先进方法。

Conclusion: Ring Forcing通过环形检索训练策略和存储扩展技术，有效解决了长时视频生成中物体恒存性不足和记忆容量受限的瓶颈，实现了分钟级视频的长期连贯性与物体身份保持。
...
TL;DR: 提出AVTP框架，一种训练无关的自适应视觉Token剪枝方法，适用于多种LVLM架构，在多图像场景中实现了2倍推理加速同时保持约96%的原始精度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Token剪枝方法多依赖静态策略，难以适应不同LVLM架构和多图像场景，且受限于注意力计算，无法兼容FlashAttention等高效技术，导致计算和上下文长度瓶颈难以有效缓解。

Method: AVTP框架为训练无关方法，通过经验分析不同LVLMs的视觉注意力分布来确定剪枝层，并在多图像场景中实现自适应剪枝比例——重要性更高的图像保留更多Token。

Result: 在多项多图像基准上，Qwen3VL-8B实现2倍推理加速并保持96.1%精度，InternVL3.5-8B保持94.1%精度，LLaVA-OV-7B甚至超过原始基线性能。

Conclusion: AVTP作为通用且鲁棒的训练无关视觉Token剪枝框架，能有效适配不同LVLM架构，在多图像场景显著提升推理效率，同时保持甚至超越原始模型精度。
...
TL;DR: 本文提出了多指令多镜头长视频编辑（MMLVE）任务及对应的Agent框架和评测基准，旨在解决长视频编辑中实体碎片化、编辑幻觉和时序断裂等核心挑战。通过LLM与VLM协同的智能体实现镜头级视频解耦和指令精确解析，在MMLVE-Bench上超越Seedance 2.0等闭源SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成式AI视频编辑方法主要聚焦单镜头或短视频片段，面对长视频多指令编辑时，朴素的分块策略（如固定时长切分）会导致实体碎片化、严重的编辑幻觉以及时序连续性破坏。这一研究空白亟需填补。

Method: 提出MMLVE-Agent智能体编辑框架，利用大语言模型（LLM）与视觉语言模型（VLM）的协同作用，实现镜头级视频解耦（shot-level video decoupling）和精确指令解析（precise instruction parsing）；同时构建MMLVE-Bench数据集，包含复杂真实世界时空动态、高密度异构指令和稀疏随机实体分布，并设计了三个面向MMLVE的评测指标。

Result: 大量实验表明MMLVE-Agent在MMLVE-Bench上优于Seedance 2.0等现有闭源SOTA方法，成功消除编辑幻觉、保持跨镜头编辑一致性，并实现无缝的时空转换。

Conclusion: 通过将长视频编辑建模为多指令多镜头的结构化任务，结合LLM+VLM的智能体协作，可以有效解决长视频编辑中的一致性与幻觉问题，为该领域提供了新的任务定义、方法与评测基准。
...
TL;DR: 本文提出HyDiff-EI框架，一种无需大规模预训练的测试时优化扩散模型，通过嵌入等变一致性约束来解决高光谱图像（HSI）修复问题，在真实数据集上展示了优于现有自监督和扩散方法的表现。


<details>
  <summary>Details</summary>
Motivation: 传统扩散方法依赖大规模预训练数据，但遥感场景中大规模标注高光谱数据集稀缺且传感器配置多样，需要一种灵活、无需大型数据集的HSI修复方案。同时无监督修复是病态问题，需要引入物理先验约束。

Method: HyDiff-EI是一个测试时优化框架，直接从单个受损HSI采集样本中学习，无需大规模预训练。方法在扩散过程中嵌入等变一致性约束，利用高光谱图像固有的几何对称性和内在特征，将生成式扩散建模与自洽物理先验相结合。

Result: 在Chikusei、Botswana和EMIT等真实数据集上的大量实验表明，HyDiff-EI在无噪声和有噪声情形下均显著优于现有的自监督和基于扩散的图像修复算法，且等变先验的耦合大幅提升了噪声鲁棒性和泛化能力。

Conclusion: HyDiff-EI成功弥合了生成式扩散建模与自一致物理先验之间的差距，为实际遥感场景中数据受限条件下的HSI修复提供了有效且灵活的新范式。
...
TL;DR: LLaVAFlow是一个信息论蒸馏框架，通过压缩和最大化互信息来保留多模态大语言模型（MLLM）中隐含的跨模态对齐流，从而缓解视觉指令微调中的灾难性遗忘，同时提升下游任务性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: MLLMs虽然具有强大的泛化能力，但视觉指令微调会导致灾难性遗忘，损害整体泛化。现有方法仅通过调节权重更新来减少遗忘，却忽视了MLLM中根本性的跨模态对齐问题。

Method: LLaVAFlow包含两个关键步骤：(1) 压缩提取关系与MLLM嵌入之间的互信息，促使可学习模块产生有利于下游任务的精炼对齐流；(2) 最大化预训练与微调后MLLM提取对齐流之间的互信息，实现紧凑对齐信息的迁移。

Result: 大量实验表明，LLaVAFlow是一个有效的即插即用框架，能够保留对齐流，同时增强下游任务性能和泛化能力。

Conclusion: LLaVAFlow验证了跨模态对齐隐含在信息压缩轨迹中的观点，通过信息论蒸馏有效缓解了灾难性遗忘问题，并为MLLM微调提供了一种新颖的即插即用解决方案。
...
TL;DR: 本文实证研究了自适应ECG导联选择策略的价值依赖于下游诊断评估器（evaluator）的程度。通过在PTB-XL上冻结两种策略（ECG-on-Demand和MGA）的采集轨迹，分别用受控逻辑评估器和更强的ResNet1D评估器打分，发现所有30个交互效应估计均为正且置信区间排除零，表明自适应策略相对于固定策略的优势在更强评估器下发生逆转（从-0.011变为+0.029）。敏感性分析排除了参考选择和掩码分布等伪影解释，提示ECG通道分配应与诊断骨干联合开发验证。


<details>
  <summary>Details</summary>
Motivation: 自适应ECG导联选择策略（如ECG-on-Demand和MGA）通过个性化分配通道预算来超越固定的群体协议。然而，这些策略的价值是用下游诊断评估器定义的——在某个评估器下学习到的边际效用，当评估器更换时未必能迁移。理解这种评估器依赖性对于可靠地部署自适应采集策略至关重要。

Method: 在PTB-XL数据集上，作者冻结了两个用受控任意掩码逻辑评估器训练的采集策略（ECG-on-Demand和MGA），然后用更具预测力的掩码原始波形ResNet1D对其不加改变的采集轨迹进行评分。通过穷举搜索为每个评估器分别提供指标匹配的群体固定比较器，形成干净的交互对比。同时进行了三项事后敏感性分析：公共参考评分、混合掩码训练强评估器，以及评估器对齐的Strong-MGA策略训练。

Result: 在预算k=4的保留评估集上，ECG-on-Demand从受控评估器下的D_C=-0.011（偏向自适应）转变为强评估器下的D_S=+0.029（偏向固定），NLL交互效应为+0.041（95% CI [+0.030, +0.050]）。两个策略×五个预算×三个概率指标共30个交互估计全部为正，配对置信区间均排除零。三项敏感性分析均保持正交互区间，但评估器对齐训练可缩小但无法消除差距。

Conclusion: 自适应ECG通道分配策略应与其预期的诊断骨干联合开发和验证。评估器依赖性是一个实质性的、非伪影的现象，联合优化的感知-诊断系统仍是一个未解决的开放问题。
...
TL;DR: 本文批判了当前AI图像处理研究中的"模型优先"模式（即直接从公开模型和基准数据集出发进行优化），主张转向"问题优先"的研究范式，通过区分物理成像问题、解决方案原理、统计估计器和计算实现四个层次，提出六阶段工作流程，并呼吁在研究文化、教育、证据标准方面进行系统性变革。


<details>
  <summary>Details</summary>
Motivation: 现代AI使图像处理能力大幅提升，但模型、数据集和基准排行榜的易获取性催生了"模型优先"的研究模式——研究者从现成架构出发在公开基准上优化，而非从真实成像问题出发。这导致基准成绩优异却未必增进对真实问题的理解或解决，需要一种问题驱动的替代范式。

Method: 本文采用论证与案例研究法：首先提出区分物理成像问题、解决方案原理、统计估计器和计算实现的概念框架；随后通过超分辨率与低光增强两个案例，展示基准数据集如何与真实问题产生偏差；进而提出包含问题表述、图像采集、信息损失分析、假设、歧义性、评估在内的六阶段工作流程，并提出关于证据、可复现性、不确定性和SOTA表述的更清晰标准。

Result: 论文揭示了基准任务与真实成像问题之间存在显著差异的性能表现被误读的可能性，提出了一套完整的问题优先研究框架（六阶段流程），并给出了改进证据标准、可复现性、不确定性和SOTA声明要求的制度化建议。

Conclusion: 要实现对成像问题的深刻理解并推动真正的科学与技术进步，必须改变研究文化和教育方式，使未来研究者以问题为先导，将现代AI作为解决真实问题的工具而非研究的起点。
...
TL;DR: 提出了MedFG-VQA，一个轻量级医学视觉问答框架，通过记忆库增强DCT低频特征并结合图增强交叉注意力实现视觉-文本对齐，同时构建了包含200万以上问答对的大型合成数据集SynMed-VQA，在多个基准上以更低计算成本达到与更大模型相当或更优的性能。


<details>
  <summary>Details</summary>
Motivation: Med-VQA在临床决策支持中具有重要前景，但面临两大挑战：标注数据稀缺以及现有大型视觉语言模型计算开销过高，限制了其临床部署潜力。

Method: 提出MedFG-VQA轻量级框架，包含两个关键组件：(1) 频率-记忆融合（FMF），基于DCT分解从可学习的记忆库中检索以增强低频特征；(2) 图感知交叉注意力（GACA），通过交叉注意力对齐视觉-文本特征并通过图卷积聚合进行细化。此外，利用GPT-4o构建了包含200万以上问答对、覆盖9种影像模态和10个主要器官的合成数据集SynMed-VQA以缓解数据稀缺问题。

Result: 在SynMed-VQA和三个标准生物医学VQA基准上的大量实验表明，MedFG-VQA相比更大的模型实现了具有竞争力或更优的性能，同时保持显著更低的计算成本。

Conclusion: MedFG-VQA通过轻量化设计和高效的特征增强与对齐机制，在医学VQA任务中实现了高效能和低计算开销的平衡，展示了其在临床部署中的潜力。
...
TL;DR: 本文提出MedREAL，一个统一的多模态医学视觉问答与定位框架，通过Seg Anchored Reasoning Pooling（SARP）从MLLM的[SEG] token中蒸馏语义证据，并利用Reasoning-to-Visual（R2V）融合机制将推理感知特征注入分割流水线。同时构建了包含13,824个专家验证样本的MedRAVS-13K数据集，在基准上达到68.49% gIoU和70.47% cIoU，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）在医学视觉问答中表现优异，但其依赖全局图像特征，缺乏精确的像素级定位能力，限制了临床可信度。现有方法在高层临床推理与空间定位之间存在语义鸿沟，亟需一个能够将语言推理与空间定位无缝对齐的框架。

Method: MedREAL框架包含两大核心创新：（1）Seg Anchored Reasoning Pooling（SARP），直接从MLLM隐藏状态中的[SEG] token蒸馏任务相关的语义证据；（2）Reasoning-to-Visual（R2V）融合机制，将推理感知特征有效注入分割流水线以进行精确的掩码解码。此外，构建了MedRAVS-13K数据集，包含跨越四种成像模态的13,824个专家验证样本。

Result: 在基准评估中，MedREAL显著优于现有最先进方法，达到68.49%的gIoU和70.47%的cIoU，证明其在推理与定位联合任务上的出色表现。

Conclusion: MedREAL通过生成与文本诊断严格一致的证据掩码，为推理驱动的医学图像分析提供了一个鲁棒、可解释的框架，有效弥合了临床推理与空间定位之间的语义鸿沟。
...
TL;DR: 本文提出一种数据驱动的物体姿态估计优化方法，通过主轴对齐（基于惯性属性）将物体坐标系与其几何主轴对齐，形成物理上合理的旋转表示，从而在数据集层面提升姿态估计精度，无需修改任何网络架构。


<details>
  <summary>Details</summary>
Motivation: 现有物体姿态估计研究以模型为中心，专注于架构创新和后处理优化，忽略了数据层面（如旋转表示）的改进潜力。作者希望通过数据驱动的几何优化来提升姿态估计性能，避免复杂的网络重新设计。

Method: 提出主轴对齐（Principal Axes Alignment）旋转表示方法：基于物体的惯性属性推导其几何主轴，将物体坐标系与固有几何轴对齐。该方法在数据集层面纯优化，具有三个关键特性：(1) 固有稳定性——利用主轴能量最小化性质，对噪声和遮挡鲁棒；(2) 对称感知规范化——在数据层显式解决对称物体的旋转歧义，消除训练中的标签混淆；(3) 框架无关性——纯数据级优化，可与现有网络即插即用。

Result: 在多种类别级和实例级模型上进行了验证，实验表明该方法在保持基线网络完整性的前提下，持续且显著地提升了姿态估计精度。

Conclusion: 本文建立了以几何数据为导向的姿态估计增强新方向，通过物理驱动的旋转表示优化在数据层面获得精度提升，规避了复杂网络重设计的需要。
...
TL;DR: 提出FIDA后门攻击框架，利用特征不稳定性损失增强触发特征对扰动的敏感性，使后门特征不再表现出刚性模式，从而规避基于扰动的防御，威胁自监督人脸表征系统。


<details>
  <summary>Details</summary>
Motivation: 自监督学习模型易受后门攻击，但人脸表征中此类攻击的系统性风险未得到充分研究；自监督人脸学习中身份特征的纠缠性给攻击隐蔽性带来独特挑战，现有攻击的特征模式刚性化易被扰动防御检测。

Method: FIDA框架包含两个核心组件：使用细微语义触发器进行注入；提出特征不稳定性损失（Feature Instability Loss），在攻击优化过程中沿采样的扰动方向训练编码器，增加受触发特征对扰动的敏感性，从而避免后门表现出刚性特征模式。

Result: 实验表明FIDA在评估的设置下实现高攻击成功率并保持良性效用，能有效规避基于扰动的防御，对依赖人脸分析的真实多媒体应用构成重大威胁。

Conclusion: FIDA通过解除后门特征的刚性模式，在保持攻击效果与良性效用的同时成功规避扰动防御，揭示了自监督人脸表征系统面临的新型安全威胁。
...
TL;DR: 本文提出CMPM——一个包含1,214个标注样本的中文多面板模因基准，覆盖五种结构类型，用于评估大型视觉语言模型（LVLMs）对有序视觉布局的序列感知推理能力。实验发现标准展示准确率不能证明顺序理解能力，打乱顺序后准确率大幅下降，揭示模型在顺序敏感多模态推理上的显著短板。


<details>
  <summary>Details</summary>
Motivation: 许多多模态任务不仅依赖于识别视觉元素，还依赖于它们的排列和组合顺序。互联网模因是这一问题的典型代表，其笑点往往取决于受限的阅读顺序和跨面板的图文线索。虽然LVLMs在单张图像理解上表现强劲，但它们在结构化模因布局上的序列感知推理能力——尤其在中文社交媒体场景中——尚不明确，因此需要专门的基准来系统评估。

Method: 构建CMPM基准，包含1,214个标注样本，覆盖五种结构类型、顺序依赖、面板顺序约束和可选评论上下文。设计两层评估框架：任务1探测结构类型和顺序敏感的面板排序（含上下文消融设置），任务2评估中文模因解释生成，采用五个1-3级Likert维度（视觉、面板、幽默、上下文、忠实度）的人工评分。在统一协议下对五个代表性LVLM进行基准测试。

Result: 标准展示条件下的准确率不能作为顺序理解能力的充分证据：在主要的面板打乱条件下，准确率出现显著下降，揭示了模型在顺序敏感多模态推理上存在持续性差距。任务2的人工偏好显示Gemini 3.1 Pro和GPT-5.5优于开源模型，而评论上下文仅为核心四项指标带来微小且不一致的提升。

Conclusion: 现有LVLMs在中文多面板模因的顺序感知推理方面仍存在明显不足，尤其是在面板顺序被打破时表现大幅退化，说明模型并未真正理解视觉布局的序列依赖关系。未来需要更关注顺序推理能力的建模与训练。
...
TL;DR: CGS-SLAM是一个混合去中心化/中心化的多智能体3DGS SLAM系统，仅使用RGB和惯性数据即可实现多智能体的光度重建与定位，无需RGB-D输入。


<details>
  <summary>Details</summary>
Motivation: 现有基于3DGS的SLAM方法大多依赖RGB-D输入，这在消费级智能手机上不可用；同时很少有方法将3DGS集成到多智能体协作框架中。因此需要一种仅依赖RGB和惯性数据、支持多智能体协作的3DGS SLAM系统。

Method: 采用混合去中心化/中心化架构：每个智能体利用惯性数据作为运动先验进行局部跟踪，并使用度量单目深度估计器（Depth Pro）重建尺度化地图；智能体间共享关键帧编码，在空间重叠区域实现动态关键帧选择以增强子图对齐；中心服务器使用VGGT作为视图对齐模型进行子图对齐；双向通信在映射过程中保持低通信开销。

Result: 在多个数据集上的实验表明，该方法在跟踪性能上具有竞争力，渲染质量优于现有最先进方法，且子图对齐精度高。

Conclusion: CGS-SLAM证明了仅使用RGB和惯性数据的多智能体3DGS SLAM是可行的，并在GNSS受限环境下实现了低通信成本的高质量全局重建。
...
TL;DR: Self-OPD是一种无需教师模型的流匹配模型蒸馏框架，通过学生自身的随机探索（K个SDE候选分支）生成逐步监督信号，利用拉-推目标优化速度场，在单目标和混合奖励基准上均超越先前的强化学习与在线策略蒸馏方法。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏（OPD）在LLM和流匹配模型中取得显著成功，但存在两个关键问题：（1）为每个新目标单独训练任务专属教师模型计算成本高昂；（2）教师与学生分布之间的差异会导致沿生成轨迹的复合误差累积。作者旨在消除对预训练教师的依赖，同时缓解分布不匹配问题。

Method: Self-OPD框架的核心创新包括：（1）将确定性的下一步状态预测分支为K个随机SDE候选，并用ODE采样器展开；（2）与确定性自参考基线比较奖励以计算归一化优势；（3）采用全分支拉-推目标，高优势分支吸引学生、低优势分支排斥学生，并加入方向感知衰减和SDE方差归一化；（4）在奖励层面融合多目标归一化分数，避免直接的梯度冲突。

Result: 在单一奖励和混合奖励基准上的实验表明，Self-OPD在无需任务专属教师的情况下，性能优于先前的强化学习和在线策略蒸馏方法。

Conclusion: Self-OPD通过将学生的自我探索转化为逐步监督信号，成功消除了在线策略蒸馏对任务专属教师模型的依赖，同时解决了分布不匹配带来的复合误差问题，为流匹配模型提供了一种更高效、更通用的对齐范式。
...
TL;DR: TetherMem是一个无需训练的、查询感知的时空记忆路由器，用于冻结的视频生成器。它通过分离主体与场景查询，并以区域和年龄条件先验调节历史访问，解决了流式自回归视频模型中"记忆锚定场景欠进展"的问题——即背景、视点和场景结构被锁定在先前生成状态中的失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有流式自回归视频模型的传统方法对主体和场景查询采用相似的历史暴露策略，虽然稳定了主体，但也可能导致背景、视点和场景结构被锁定在先前生成状态，即使局部运动仍在继续。这种"记忆锚定场景欠进展"问题仅靠一致性指标和运动指标无法检测，需要专门的解决方案。

Method: TetherMem采用查询感知的时空记忆路由器，将主体查询与场景查询分离。对主体查询保留承载身份信息的历史记忆，而对场景查询则减少对主体历史和陈旧背景的依赖，通过区域和年龄条件先验来调节历史访问。

Result: 在10位标注者的2,400个盲对比较判断中，TetherMem在8个流式长视频基线中获得了最高的整体质量（0.780）和场景进展（0.769）预期偏好估计。在完整的30秒视频上，它能持续维持背景、视点和场景状态的变化，同时保持主体的可识别性和时间连续性。

Conclusion: TetherMem证明了通过查询感知的历史访问调节，可以在不断言主体一致性的同时实现更自然的场景演进，为流式长视频生成提供了一种有效的无需训练改进方案。
...
TL;DR: 发布了AraMS-28k，这是迄今最大的公开真实历史阿拉伯语手稿行级数据集，包含14本书、3,043页和28,600条标注文本行，涵盖Naskh、Ruq'ah和Maghrebi三种手写字体传统及一种石印印刷版本。每行标注为正文或页边注，并首次提供页边注的插入锚点注释以恢复手稿的真实非线性阅读顺序。数据集通过RefLAM流水线构建（多模态LLM OCR与独立转录对齐+人工审查），提供原始带音符转录和去音符版本，并附Kraken和HATFormer的HTR基线结果，以CC BY-NC-SA 4.0许可证发布。


<details>
  <summary>Details</summary>
Motivation: 历史阿拉伯语手稿识别面临数据稀缺问题，现有数据集规模有限且缺乏行级页边注和阅读顺序标注。作者旨在填补这一空白，创建一个大规模的、带丰富标注（主文本/页边注分类、插入锚点、阅读顺序恢复）的真实历史手稿数据集，以支持HTR、布局分析和阅读顺序恢复等可复现研究。

Method: 构建了RefLAM（reference-grounded annotation pipeline）标注流水线：将多模态LLM的OCR输出与独立来源的干净转录进行对齐，且每一行都经过人工审核，结合自动验证与专家监督。生成的标注包括行级主文本/页边注标签、页边注的插入锚点、原始带变音符转录及其去音符版本。数据覆盖三种手抄字体传统和一种石印版本。

Result: 形成了AraMS-28k数据集（27,971条正文本行、629条页边注行），报告了语料库和每本书级别的统计信息，提供了使用Kraken和HATFormer的HTR基线结果，并展示了从分布内页面到完全未见书籍的跨字体泛化梯度。

Conclusion: AraMS-28k是首个包含页边注插入锚点注释（实现行级阅读顺序恢复）的历史阿拉伯语手稿数据集，并以固定划分和CC BY-NC-SA 4.0许可发布，支持研究社区在阿拉伯手稿识别、布局分析和阅读顺序恢复方面开展可复现研究。
...
TL;DR: KISS-GS 提出了一种模块化、与训练完全解耦的3DGS压缩流水线，通过剪枝压缩（15.7x）和基于图像格式的编码（6.6x），可选编码感知微调（2.2x），实现总体85x至319x的压缩，并超越紧耦合的集成方法。


<details>
  <summary>Details</summary>
Motivation: 3DGS重建虽流行但其未压缩文件体积巨大，部署困难。现有压缩系统将多种策略组合在一起，掩盖了性能增益来源，且限制了组件在不同训练流水线间的复用。因此需要一种将压缩与训练完全解耦、模块化且各阶段增益透明的压缩方案。

Method: 基于"保持简单"原则构建模块化压缩流水线：先对标准vanilla 3DGS重建场景采用先进的剪枝方案进行压缩（15.7x）；再提出SOG-XT格式将其编码为图像格式，包含两个核心贡献：(i) 自组织2D码本（Self-organizing 2D Codebooks）和 (ii) 并行代表分配平滑（PRAS），利用四元数与尺度参数化的对称性生成更利于编码的2D属性网格，实现6.6x压缩；可选地加入编码感知微调进一步获得2.2x增益。解码仅依赖Web原生图像格式。

Result: 在标准3DGS基准上，与未压缩的vanilla 3DGS相比，总体场景大小压缩达85x至319x，为真实场景设立了新基准，在率失真性能上超越了紧密集成的压缩方法。

Conclusion: KISS-GS通过模块化、解耦训练的设计使压缩各阶段增益透明化，且每个阶段易于与未来重建和压缩方面的进展结合。解码依赖Web原生图像格式，具有简单、普适、易部署的特点。
...
TL;DR: 提出一种无需训练的过滤方法，用于前馈3D高斯泼溅重建中去除瞬态物体（仅在部分视图中出现的物体）导致的伪影，从而提升新视角渲染质量。


<details>
  <summary>Details</summary>
Motivation: 前馈3D高斯泼溅重建在随意拍摄场景中常含有瞬态物体（只在部分视图中观察到的内容），这些物体会被编码到对应的逐视图高斯中，并在组合表示中残留，导致新视角出现模糊、重复或漂浮伪影。

Method: 利用前馈模型的逐视图预测结构，对每个输入视图：排除其关联高斯后用剩余表示渲染同一相机，与原始图像对比获得与其余输入不一致的内容区域；通过特征相似度形成候选区域，再通过渲染验证仅保留那些移除后能降低其他输入视图重建误差的候选区域。整个过程在单个冻结的预测上操作，无需重新训练或场景特定优化。

Result: 在三个重建模型和两个干扰物基准上一致地提升了新视角质量（在不同输入视图数量下）；在干净场景上，四个模型的评估显示原始重建基本得以保留。

Conclusion: 该方法有效地识别并移除了瞬态物体产生的不一致高斯，在保持干净场景重建质量的同时显著减少了瞬态物体带来的伪影，且无需额外的训练或优化过程。
...
TL;DR: RubricRM提出了一种成对生成式奖励建模框架，通过为每个输入生成包含评估维度、权重和评分标准的具体化规范（rubric），再基于该规范对候选图像进行评分，用于文本到图像生成和指令式图像编辑任务的视觉对齐。实验表明其在多个基准上超越现有专用奖励模型，并可与强大的私有MLLM裁判竞争。


<details>
  <summary>Details</summary>
Motivation: 现有视觉奖励模型大多使用单一标量分数或固定评估标准，无法适应不同指令的评估需求，这限制了解释性和任务敏感性。文本到图像生成和图编辑等任务中，不同输入需要不同的评估维度，因此需要一种能生成输入特定评估标准的奖励模型。

Method: 提出对生成式奖励建模框架RubricRM：首先生成输入特定的rubric（包含评估维度、权重和评分标准），再依据rubric对候选图像评分。采用两阶段训练流程：监督微调（SFT）教授模型基于rubric的评分范式，GRPO通过细粒度维度级奖励进一步提升评分能力。分别针对文本到图像生成和图像编辑训练专用模型。

Result: 在多个生成和编辑基准上的实验表明，RubricRM优于现有的专用奖励模型，并且在使用更小骨干网络的情况下仍可与强大的专有MLLM裁判（proprietary MLLM judges）保持竞争力。

Conclusion: 输入特定的rubric生成能够提升奖励模型的解释性和任务敏感性，两阶段训练（SFT+GRPO）有效地提升了评分性能。该框架在较小模型规模下即可达到与大型私有裁判模型相当的竞争力。
...
TL;DR: Geo-LoRA 是一个几何感知的框架，用于无重放类增量学习。它通过子空间投影保持（SPP）、自适应核松弛对齐（ACSA）和标准差校准块重叠（MCBO）三种约束，分别规范共享和任务特定的 LoRA 低秩子空间在流形上的演化，防止任务更新坍塌到先前占据的方向，在多个基准数据集上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 无重放的类增量学习（CIL）结合 LoRA 适配器仍然面临挑战：不同任务更新的低秩子空间缺乏几何控制，导致共享表征不稳定，且任务特定更新反复坍塌到先前已占据的方向，损害持续学习的能力。

Method: Geo-LoRA 包含三条约束路径：(1) 共享分支采用 SPP 约束连续更新在 Grassmann 流形上沿平滑轨迹演化；(2) ACSA 将过渡分解为主成分和残差成分，对齐前者、调制后者以平衡稳定性和可塑性；(3) 任务特定分支采用 MCBO 通过归一化投影重叠施加统计约束，惩罚过度复用以减少子空间拥挤。所有约束无需引入额外适配器类型，仅基于标准 LoRA。

Result: Geo-LoRA 在多个基准数据集和不同任务长度下一致取得最先进的性能，验证了几何约束对规范 LoRA 子空间演化在持续学习中的有效性。

Conclusion: Geo-LoRA 为持续低秩适配提供了原则性的几何公式，通过显式调控共享与任务特定 LoRA 子空间的演化，在不增加适配器种类的前提下实现了稳定与可塑性的平衡，为无重放 CIL 设立了新的性能基准。
...
TL;DR: 本文发现了图像到视频（I2V）生成模型中的时间维度漏洞，并提出TempJail框架，通过时间抽象（将恶意语义分解为初始帧视觉条件和时间文本指令）与语义伪装（图像侧潜在扰动+文本侧重写为无害模板）实现对商业I2V模型的时序越狱攻击，在GPT与人工评估下分别提升23.3%和22.0%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有对I2V模型的越狱攻击研究主要聚焦于单帧违规，忽视了视频生成模型独特的时间维度。作者发现I2V系统中存在时间漏洞：不安全语义并非从单一帧产生，而是通过时间维度上的语义组合逐渐浮现，这一安全盲区亟需系统性研究和防护。

Method: 提出TempJail框架，包含两个核心策略：（1）时间抽象：将恶意目标描述分解为初始帧视觉条件和时间文本指令两部分；（2）语义伪装：图像侧通过预训练编码器梯度引导，将语义注入建模为扩散采样中受控的潜在空间扰动；文本侧将恶意描述重写为无害的'主体-动作-场景'模板，绕过安全过滤器同时保留时间引导信息。在黑盒推理阶段，两模态协同使恶意语义随时间逐步触发。

Result: 在Kling、Seedance、Veo和PixVerse等闭源商业模型上验证，TempJail相比现有最优方法在GPT-5.2评估下攻击成功率提升23.3%，在人工评估下提升22.0%。

Conclusion: 该研究揭示了I2V系统在时间维度上独特且尚未被充分认知的安全漏洞，证明恶意语义可以通过时序组合逐步呈现。研究同时提出了有效的攻击框架TempJail作为安全评估工具，为推动I2V模型在时间维度上的安全防护机制建设提供了重要参考。
...
TL;DR: 本文提出Aphanta，一个自动任务发现与闭环诊断框架，用于评估MLLM→图像编辑器→MLLM流水线的效用。研究表明图像编辑器的效用强烈依赖于任务类型，在视觉线索注入、接地和反事实状态实现等任务上增益显著，而在需要符号敏感构建或结构外推的任务上可靠性较低。


<details>
  <summary>Details</summary>
Motivation: 显式视觉中间表示可以帮助多模态大语言模型外部化空间证据和更新的视觉状态，但其实际效用取决于图像编辑器能否忠实地实现所需的变换。然而，这种效用是否普遍存在、在哪些任务上有用、哪些任务上无用，缺乏系统的评估框架。

Method: Aphanta框架通过自动化任务发现和闭环诊断，对比三种条件：直接推理、使用编辑器生成的中间表示推理、使用理想化参考中间表示推理，从而分离潜在视觉提升空间与当前编辑器的实际效用。在20个候选任务和多个编辑器-MLLM组合上进行系统评估。

Result: 效用强烈依赖于任务条件：增益集中在视觉线索注入、接地和反事实状态实现任务上；需要符号敏感构建或结构外推的中间表示可靠性明显较低。在筛选出的正效应任务子集上，Qwen流水线将平均任务分数从0.343提升至0.445（+10.2分，相对提升29.7%）。完整研究同时保留被过滤和失败的任务以揭示性能边界。

Conclusion: 图像编辑应定位为专门的视觉工作空间而非通用推理机制。Aphanta可作为可复用的协议，用于测量任务-表示对齐、编辑器实现能力和下游流水线的实际效用。
...
TL;DR: 本文综述了头皮到颅内脑电（scalp-to-intracranial）推断领域的进展，提出了以目标为中心的分类框架，区分事件推断、特征转译和波形重建三类任务，并指出当前研究仅支持部分颅内事件、低频成分和任务相关表征的推断，尚不能唯一恢复任意接触水平的神经活动。


<details>
  <summary>Details</summary>
Motivation: 颅内脑电图（iEEG）具有高时间精度和空间特异性，但其侵入性和有限的解剖覆盖限制了常规应用。这一约束推动了从头皮信号推断颅内活动的虚拟iEEG研究，但该领域缺乏统一的分析框架和系统的证据评估标准。

Method: 采用以目标为中心的综述框架，将推断任务分为事件推断、特征转译和波形重建三类，并区分可预测性与可观测性、可辨识性、保真度和实用性等概念维度。证据评估依据队列独立性、解剖与频谱覆盖、训练/测试分离以及目标患者适应性等标准进行。

Result: 当前研究支持对特定颅内事件、低频成分和任务相关表征的推断，但不支持对任意接触水平活动的唯一定向恢复。现有验证缺乏适当的对照组、源成像基线、不确定性评估和增量效用测试。

Conclusion: 未来进展依赖于独立的双模态配对数据集，以及对虚拟iEEG相对于常规头皮EEG和EEG源成像是否具有增量价值的预期性证据。
...
TL;DR: 提出MVC-Bench，一个以校准为中心的医学图像分类基准，系统评估VLM和Medical-VLM在域内和域偏移下的校准性能，并提出Multi-Class Margin (MCM)正则化方法实现最先进的校准效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注提升VLM和Medical-VLM的准确性，而医学领域的校准问题被忽视。在临床场景中，模型置信度校准至关重要，因此需要一个系统的基准来评估和改善医学视觉-语言模型的校准性能。

Method: 构建MVC-Bench基准，从三个维度评估校准：(i)对模态、骨干网络和域偏移的鲁棒性；(ii)校准策略和提示调优方法的有效性；(iii)在提示模板和随机种子变化下的稳定性。覆盖8种骨干网络、3种医学模态（眼底成像、组织病理、胸部X光），比较训练后校准、训练时校准和零样本推理方法以及6种提示调优方法，进行超过1638项受控实验，报告ECE、MCE和ACE指标，并提出MCM正则化训练时校准方法。

Result: 在超过1638项受控实验中，MCM正则化在12个域内设置中的10个达到最低ECE，在域偏移下仍保持竞争力。研究还揭示了VLM和Medical-VLM失校准的根本原因。

Conclusion: MVC-Bench为医学图像分类中的VLM校准提供了结构化的评估框架，MCM正则化方法可作为安全关键医学工作流中改善校准的有效方案，并为未来研究提供可操作的指导。
...
TL;DR: 本文提出一种全可微的抖动校正方法，用于X射线相位衬度微型CT成像。该方法利用基于深度学习的图像质量度量（VIF），直接从采集的投影数据中估计并补偿每个投影的刚性抖动，无需预扫描的无运动参考。通过梯度基自动对焦策略、3D CNN预测VIF分数及背景选择性全变分惩罚，在模拟与实验数据上验证了该方法能可靠恢复因抖动丢失的精细结构细节。


<details>
  <summary>Details</summary>
Motivation: X射线相位衬度微型CT成像中，扫描期间的机械抖动会导致投影数据中的运动伪影，破坏重建图像中的精细结构细节。现有的抖动校正方法通常需要预扫描的无运动参考图像或依赖手工设计的图像质量度量，限制了其在实际同步辐射成像场景中的适用性。

Method: 该方法包含三个核心组件：(1) 基于梯度的自动对焦策略，适配平行束几何；(2) 紧凑的3D卷积神经网络，从单一损坏体数据预测VIF图像质量分数，替代无清洁参考的需求；(3) 仅作用于图像背景的空间选择性全变分惩罚，抑制优化过程中产生的伪高频结构。整个流水线是完全可微的抖动校正框架。

Result: 在多个同步辐射束线采集的生物标本（模拟与实验投影数据）上进行了评估。结果表明该集成流水线能可靠恢复因抖动丢失的精细结构细节，并在形态学上不同的样本间表现出良好的泛化能力。基准研究验证了VIF度量对抖动伪影的敏感性。

Conclusion: 该全可微抖动校正方法在无需预扫描参考的情况下有效消除了相位衬度微型CT中的抖动伪影，恢复了精细结构信息，并通过多样化生物标本实验验证了其跨样本泛化能力。
...
TL;DR: 本文提出OCSD（Object-Conditioned Social Diffusion），一个将运动历史、多人交互和物体信息统一集成到条件扩散模型中的行人轨迹预测框架。OCSD在HiK和HOI-M3基准上达到最优性能，分别将两秒路径误差降低31.3%和33.2%。


<details>
  <summary>Details</summary>
Motivation: 在复杂场景中准确预测人群运动需要对整个环境的过去和现在状态进行推理。然而，如何将物体信息和社交互动有效整合到一个统一框架中仍然是一个挑战。

Method: OCSD是一个条件扩散模型，包含两个核心组件：(1) 物体条件化机制（object-conditioning mechanism），在每次去噪过程中调节生成过程，实现细粒度的人-物推理；(2) 社交编码器（social encoder），建模场景中所有人之间的交互。该模型天然支持变长群体规模、复杂社交互动以及采样多种可能的未来轨迹。

Result: 在Humans in Kitchens (HiK)和HOI-M3基准上达到最先进水平。相比先前工作，在HiK上将两秒路径误差降低121.5mm（31.3%），在HOI-M3上降低130.5mm（33.2%），同时生成了更逼真的长期预测结果。

Conclusion: OCSD通过将物体信息和社交互动统一集成到扩散框架中，有效解决了复杂场景下的人群运动预测问题，验证了物体条件化扩散模型在人-物-人交互建模中的有效性，并实现了更准确的短期预测和更真实的长期预测。
...
TL;DR: Video-OPSD是一个面向视频大语言模型（Video-LLMs）的自我蒸馏（self-distillation）后训练框架，利用长视频中固有的时序冗余性，将注释证据帧作为特权视觉信息同时用于教师构建和知识迁移，在多种视频理解基准上优于标准OPSD，并在更短训练时间内达到与GRPO相当的性能。


<details>
  <summary>Details</summary>
Motivation: 在线策略自蒸馏（OPSD）在多模态模型中展现出潜力，但尚未在Video-LLMs中得到充分探索。现有方法构建特权教师的方式通常是通过向上下文添加额外信息，而非利用视频内部本身存在的"特权监督"——即长视频中只有少量关键帧包含回答问题所需的证据信息。这一观察启发作者开发一种利用视觉证据本身的OPSD框架。

Method: 提出Video-OPSD框架，包含两个核心组件：(1) 证据引导的自教师（Evidence-Grounded Self-Teacher）：教师仅基于注释的证据帧进行推理，而学生仍基于完整视频推理，从而获得更有信息量的监督；(2) 证据引导的token优化（Evidence-Guided Token Optimization）：根据每个推理token对特权视觉证据的依赖程度，自适应地对token级蒸馏损失进行加权，强化感知基础推理。

Result: 在多个视频理解与推理基准上的实验表明，Video-OPSD在多个骨干网络上都一致优于标准OPSD，性能与GRPO相当，但所需训练时间显著更少。

Conclusion: Video-OPSD通过挖掘视频内部固有的特权视觉证据，为Video-LLMs提供了一种有效且高效的后训练方案，在保证性能的同时大幅降低了训练成本。
...
TL;DR: 本文提出一种计算感知的多任务评估协议，用于评估计算机视觉中的隐私增强技术（PETs），弥补仅用图像分类评估PETs过于简单的问题。该协议结合轻量级代理任务，互补地覆盖视觉结构的多个方面，实验表明分类准确率相近的PETs在其他任务上可能表现差异显著。


<details>
  <summary>Details</summary>
Motivation: 现有PETs评估主要依赖图像分类，但分类任务仅捕捉语义可分性，对几何变换、空间布局或局部边界变化不敏感，难以作为通用视觉任务的代理。而穷举式下游任务评估计算成本过高，因为每种PET变换和参数设置都需要重新训练模型。因此需要一个既全面又计算高效的评估协议。

Method: 提出一个计算感知的多任务评估协议，包含轻量级代理任务，这些任务针对视觉结构的互补方面（如语义、几何、空间布局、局部边界等），同时保持计算简单快速。在不可逆隐私变换、基于密钥的块级基元以及可学习的图像加密方案上进行了评估验证。

Result: 实验表明，具有相似分类准确率的PETs在其他视觉任务上可能表现出显著差异，说明仅依赖分类评估会产生误导性结论，多任务评估能够揭示PETs在不同视觉结构维度上的真实表现差异。

Conclusion: 研究强调了PET评估协议需要超越仅基于分类的报告范式，采用计算感知的多任务评估来更全面地衡量隐私保护与任务性能之间的权衡。
...
TL;DR: SpatialCrafter是一个两阶段框架，通过引入全局3D代理（global 3D proxy）解决基于视频扩散模型（VDM）的图像到场景生成任务中存在的随机幻影、长期漂移和3D一致性差的问题，在合成与真实世界数据集上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视频扩散模型的图像到场景生成方法依赖稀疏点云或2D全景图等不完整的条件信号，导致生成结果存在随机幻影（stochastic hallucinations）、长期漂移（long-term drifts）和3D一致性欠佳的问题，限制了在游戏、机器人、虚拟现实等领域的应用。

Method: SpatialCrafter采用两阶段框架：（1）全局代理生成阶段，提出点锚定稀疏结构流（PaSS Flow）模块，预测空间对齐且几何一致的3D代理；（2）外观细化阶段，将视频扩散模型重构为生成式延迟细化器（Generative Deferred Refiner），在代理定义的场景几何上合成高频逼真细节。同时引入并行几何注入（Parallel Geometry Injection）和代理感知损坏训练（Proxy-Aware Corruption）策略，以更好地将代理与预训练的VDM集成，且不破坏预训练生成流形。此外，构建了包含115K场景的大规模混合数据集。

Result: 在合成和真实世界数据集上的大量实验表明，SpatialCrafter在各项指标上优于现有最先进方法，能有效缓解长期漂移问题，并在快速相机运动和极端视角变化下保持鲁棒性和一致性。代码、模型和新构建的数据集将公开发布。

Conclusion: 通过引入全局3D代理作为中间表示，并配合两阶段生成框架和针对性的训练策略，SpatialCrafter为可探索的图像到场景生成提供了一种高质量、高一致性的解决方案，同时首个混合数据集的构建为该领域后续研究提供了重要资源支撑。
...
TL;DR: 本文针对AMD和DME的四条OCT病灶分割管线（2D/3D变体）进行系统消融研究，在域内验证集上Dice达0.76–0.82且具有良好的体积与表面校准（r≥0.97），并通过代理指标框架（生物标志物AUROC、CST相关性、纵向一致性）在无病灶标签的外部临床队列OLIVES上验证跨域泛化能力，结果表明预测可追踪域外临床生物标志物但强度弱于域内，属于支持性而非验证性证据。


<details>
  <summary>Details</summary>
Motivation: OCT深度学习分割模型大多仅在域内验证，缺乏在不同采集协议下的临床数据泛化测试，这限制了其临床可用性的可信度评估。

Method: 开发并系统消融四条病灶分割管线（AMD和DME各含2D与3D变体），建立全容积、校准感知的采用标准，利用集成组合策略提升性能；在外部临床队列OLIVES上采用无病灶级标签的代理指标框架（生物标志物AUROC、CST相关性、纵向一致性）评估泛化能力。

Result: 域内验证Dice为0.76–0.82，体积与表面相关度r≥0.97；跨域评估显示预测与临床生物标志物可追踪但相关性弱于域内，集成组合被识别为最稳定的性能提升因子。

Conclusion: 模型在域外数据上能够追踪临床生物标志物趋势，为自动化病灶负担追踪提供了支持性证据，但尚不足以构成对临床工具的完全验证，需要更多关注跨协议泛化评估的方法论。
...
TL;DR: 本文提出EditaLive，一个面向实时流媒体场景的流式角色视频编辑框架。它基于预训练图像动画模型Wan-Animate，通过参考帧编辑和视频重建适配为指令驱动的人像视频编辑模型，并采用对齐自卷蒸馏将模型压缩为两步采样器，同时将离线双向生成改为因果流式生成，结合固定RoPE、对齐强制和首帧保留稀疏注意力机制，实现面部表情保真度高且低延迟的实时流式推理。


<details>
  <summary>Details</summary>
Motivation: 传统视频编辑主要关注场景级内容，而直播场景更强调人物主体。直接将现有视频编辑方法应用于以人为中心的直播存在两个问题：一是可能导致面部表情不一致，二是依赖多步离线推理，无法满足实时交互需求。

Method: 1) 以Wan-Animate（天然解耦外观与运动的预训练图像动画模型）为基础模型，通过参考帧编辑与视频重建，利用CharEdit-50K数据集将其适配为指令驱动的人像视频编辑模型；2) 将模型从离线双向生成改为因果流式生成；3) 设计对齐自卷蒸馏（aligned self-rollout distillation）策略，压缩为两步采样器，其中固定RoPE和对齐强制（align forcing）减少训练与推理的差异，首帧保留稀疏注意力（first-frame preserved sparse attention）过滤冗余历史信息以减轻外观漂移。

Result: 大量实验表明，EditaLive在编辑性能上达到最先进水平，同时实现了面部表情的忠实保持和低延迟的实时流式推理。

Conclusion: EditaLive证明了通过蒸馏压缩、因果流式改造与注意力机制优化，可以将基于指令的视频编辑模型适配到实时直播场景，在保证编辑质量和表情一致性的同时满足实时交互的延迟要求。
...
TL;DR: 本文提出了ReViCo（Real Visual Correction）基准，通过一个新的视觉文本纠错任务来评估视觉语言模型（VLM）对图像中文字的理解能力。实验发现即使最好的VLM与人类表现之间仍存在显著差距，且多数模型在感知视觉文本时存在困难，导致频繁出错。


<details>
  <summary>Details</summary>
Motivation: 尽管VLM在通用视觉任务中表现出色，但它们对图像内文本的深层理解仍然不足。现有基准缺乏专门针对视觉文本纠错这一需要将文字与其周围视觉上下文结合理解的任务，因此需要一个新的基准来系统评估和推动VLM在这方面的能力。

Method: ReViCo基准通过让模型识别并修复真实世界图像中的文字错误来评估VLM的文本理解能力。作者采用两种评估范式：基于提示词（prompt-based）的策略和针对性的模型训练（targeted model training），并以此对多种VLM进行横向评测。

Result: 实验表明，即使是性能最优的VLM与人类表现之间仍存在显著性能差距；进一步分析发现，大多数模型难以准确感知图像中的视觉文本，导致产生频繁的纠错错误。

Conclusion: ReViCo揭示了当前VLM在视觉文本理解上的关键短板，为开发更鲁棒、更具文本感知能力的VLM提供了新的基准基础。
...
TL;DR: 本文介绍Magpie，一个用于交互式游戏的实时生成式世界渲染系统。该系统将游戏玩法执行与视觉生成分离：游戏引擎负责解析玩家动作并维护世界状态，独立的渲染服务器基于引擎输出的白盒帧生成视觉输出，从而在保留游戏玩法可设计性和可复现性的同时，降低早期游戏原型对完整视觉资产的依赖。


<details>
  <summary>Details</summary>
Motivation: 传统游戏开发依赖常规图形管线，高质量视觉内容的制作（建模、材质、动画、光照、特效、运行时优化）成本高昂且拉长了原型开发周期。虽然视频基础模型正在改变影视制作，但游戏是非线性媒体，不仅需要连续逼真的画面，还需要稳定可复现的游戏规则、对象状态和交互结果，这促使研究者探索将生成模型应用于实时游戏渲染的系统级方案。

Method: Magpie将游戏玩法执行与视觉生成解耦。设计者在游戏引擎中定义场景和规则；运行时，游戏引擎负责解析玩家动作并维护世界状态，独立的渲染服务器则利用引擎产生的白盒帧生成视觉输出，实现生成模型在实时游戏渲染中的应用。

Result: Magpie提供了一种将生成模型应用于实时游戏渲染的系统级实现路径，能够保留游戏玩法的可设计性和可复现性，同时降低早期游戏原型对完整视觉资产的依赖。

Conclusion: Magpie为生成式AI辅助游戏渲染提供了一个系统级架构方案，通过将玩法逻辑与视觉生成分离，兼顾了游戏的交互性/可复现性与生成画面的灵活性，为降低游戏原型制作成本提供了新的技术路径。
...
TL;DR: 本文提出Ancient-Bench，一个包含2,700张图像的古中文文物文字识别综合基准，从跨千年时间（3,000年文字演变）、多介质（九类文物）和多字体（七种历史书写形式）三个维度设计，并定义了符号标准化、字符标准化和解析标准化三类注释标准以支持异构介质的一致评估。实验表明现有VLM和OCR模型在该任务上仍面临变体字、专业符号和幻觉等根本性挑战。


<details>
  <summary>Details</summary>
Motivation: 现有古中文文字识别基准存在"碎片化"问题：时间覆盖有限、介质多样性不足、文字类型不完整，导致无法全面评估当前模型在古代文物文字识别上的真实能力。

Method: 从三个维度构建基准：（1）多千年——覆盖3,000年文字演化；（2）多介质——覆盖九类文物；（3）多字体——涵盖七种历史文字形式。同时针对异构介质定义三类注释标准：符号标准化、字符标准化和解析标准化，保证评估的一致性和公平性。

Result: 在Ancient-Bench上对通用视觉语言模型（VLMs）和OCR专用模型进行广泛实验，结果显示古中文文物文字识别仍未被根本解决，主要挑战在于变体字符、专业符号识别以及幻觉问题。

Conclusion: Ancient-Bench作为一个多维度的综合基准，填补了现有基准在时间跨度、介质多样性和文字类型完整性方面的空白，为未来古中文文物文字识别研究提供了标准化的评估平台。
...
TL;DR: 本文对Tessera这一领先的地球观测基础模型进行了时间敏感性的受控研究，通过改变观测窗口（从全年到单日）评估其嵌入对土地利用/土地覆盖分类的影响，发现嵌入价值与任务相关且时间覆盖是可调成本而非固定前提。


<details>
  <summary>Details</summary>
Motivation: 许多地球观测应用需要既精确又频繁更新的土地利用/土地覆盖地图，但最强的基础模型是从一整年的观测数据构建嵌入的。需要研究这些模型在不同时间窗口下的表现，以探索近实时制图的可能性。

Method: 保持编码器冻结，在不同时间窗口（从全年到单日）重新计算Tessera的嵌入，使用线性探针和UNet分割头作为下游任务，并在LUCAS、DynamicEarthNet和PASTIS-R三个数据集上与从头训练的网络进行基准对比。

Result: 嵌入价值是任务依赖的：在PASTIS-R作物类型分类中mIoU达58.3，比最佳从头训练模型高约46%；在时间稳定的类别（如森林）中两者仅在完全监督下持平；Tessera嵌入在标签效率方面始终更优。时间窗口缩短的影响是渐进的且依赖类别，从一年缩至一个月在PASTIS-R上损失39%精度但在DynamicEarthNet上仅损失5%；单日嵌入在LUCAS上仍能达到随机水平的3.4倍。

Conclusion: 时间覆盖是可调成本而非固定前提，这为近实时制图和更快的土地利用/土地覆盖更新周期开辟了可能性。
...
TL;DR: SWIFT框架通过四种配置（SWIFT、SWIFTe、SWIFTe-LoRA、SWIFTe-LDE4）探索了直肠癌MRI分割中参数高效微调与几何精度、肿瘤检测、校准性之间的权衡。SWIFTe参数减少70.1%且检测率提升，LoRA保持性能，四成员LoRA集成获得最佳校准性能。


<details>
  <summary>Details</summary>
Motivation: 直肠癌MRI分割在自适应放疗和肿瘤反应评估中至关重要，但临床部署需要同时满足计算效率、信息丰富的校准不确定性估计和高精度分割，这三者之间存在张力。

Method: 使用在10,444个公开3D CT体素上以DINOv2风格目标预训练的Swin V2编码器，通过四种累积配置适配T2加权MRI：全量微调（SWIFT）、解码器压缩（SWIFTe）、低秩适应（SWIFTe-LoRA）和四成员LoRA解码器集成（SWIFTe-LDE4），并在247例留出测试集上评估几何精度、肿瘤检测、影像组学一致性和概率校准。

Result: SWIFTe相比SWIFT减少70.1%参数（72.8M→21.8M）并将肿瘤检测率从89.9%提升至93.9%，但表面DSC略降（0.61 vs 0.62）。肿瘤感知增强消融显示检测与边界一致性存在权衡。SWIFTe-LDE4在温度缩放后获得最佳校准（ECE=0.217，Brier=0.222），但绝对ECE仍表明存在残余校准误差。

Conclusion: 参数高效微调可在降低计算开销的同时维持或改善检测性能，但存在检测率与边界精度的权衡；集成方法改善了校准但仍未达到理想校准水平，且结果可跨预训练初始化复现但不代表外部临床泛化性。
...
TL;DR: SSMB是一种无需去模糊、自监督的运动模糊图像关键点检测器，通过局部判别增强模块恢复细粒度局部判别性，结合几何预训练和模糊感知训练两阶段策略，在关键点检测、图像匹配、相对位姿估计和视觉定位等任务上均达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 运动模糊会扭曲局部图像结构并降低关键点定位的可重复性。现有方法要么依赖计算昂贵的去模糊后检测流水线（可能引入复原伪影），要么学习回归在清晰图像上由手工设计的关键点提取器得到的位置（反映了手工检测器的假设而非模糊下真正可重复的特征）。因此需要一种不依赖手工检测器或外部伪标签的模糊不变关键点检测方法。

Method: SSMB包含三个核心组件：(1) 局部判别增强（LDE）模块，在全局特征混合后恢复细粒度的局部判别性；(2) 两阶段训练策略：第一阶段在合成形状上进行几何预训练，仅从渲染几何引导空间判别性关键点检测；第二阶段在真实清晰-模糊图像对上进行模糊感知训练，通过多组件自监督目标（交叉域一致性、几何对齐、空间覆盖）学习模糊不变检测。

Result: 在运动模糊条件下的关键点检测、图像匹配、相对位姿估计和视觉定位等任务上进行广泛评估，SSMB在稀疏关键点检测器中建立了新的最先进水平，在所有任务上一致优于有监督和自监督基线。

Conclusion: SSMB证明了无需去模糊、无需手工检测器、无需外部伪标签即可实现模糊不变的稀疏关键点检测，为运动模糊场景下的关键点检测提供了新的自监督范式。代码、模型和数据集将在论文接收后公开。
...
TL;DR: 本文提出了一种基于冗余抑制特征对齐的无监督域适应（UDA）框架，用于实现无需目标域标注的3D锥形束CT（CBCT）分割，适用于介入治疗和放射治疗场景，并在两个CT-CBCT肝脏分割基准上验证了其优于现有预训练基础模型和UDA策略。


<details>
  <summary>Details</summary>
Motivation: CBCT的3D分割在介入和放射治疗中至关重要，但面临两大挑战：带标注CBCT数据稀缺，以及CBCT与诊断CT之间存在巨大的域偏移（由采集方式、物理效应和血管对比内容差异导致），限制了跨模态模型迁移的有效性。

Method: 提出一种基于冗余抑制特征对齐的无监督域适应（UDA）框架，该框架架构无关，可同时适配基于CNN和ViT的基础模型，通过显式的特征空间桥接实现跨采集模态的泛化，无需目标域标注或推理时适应。

Result: 在两个具有挑战性的CT-CBCT肝脏分割基准（血管介入手术和放射治疗）上进行了评估，结果表明即使大规模预训练的分割网络也需要显式的特征空间桥接才能跨模态泛化，所提方法一致优于现有预训练基础模型和UDA策略。

Conclusion: 所提出的UDA框架能有效解决CBCT分割中标注稀缺和域偏移问题，并发布公共CBCT数据集的肝脏分割、代码、训练模型和权重以支持可复现性和基准测试。
...
TL;DR: 该论文以视角文章形式探讨视觉生成式AI在边缘硬件限制下的部署问题，主张从设计之初就采用软硬件协同设计方法，将部署约束纳入考量，以实现生成式AI在更广泛平台上的可持续和可访问部署。


<details>
  <summary>Details</summary>
Motivation: 视觉生成式AI在数据中心大规模文本到图像应用中迅猛发展，但在自动驾驶、农业传感器、移动设备等硬件受限的边缘场景同样重要。然而当前发展由输出质量驱动，硬件被动适应不断增长的模型需求，缺乏对参数成本和能量效率的系统考量。

Method: 作者量化评估了多种加速器平台上视觉生成模型的参数成本和能量效率，并将四类生成模型家族映射到七个真实世界应用领域，从系统层面分析模型-硬件-应用的匹配关系。

Result: 研究揭示了生成模型需求与边缘硬件能力之间的错配，展示了不同模型族在不同应用域中的部署适宜性差异，并论证了软硬件协同设计框架的必要性和可行性。

Conclusion: 主张采用软硬件协同设计方法，使部署约束在设计过程起始就被考虑，确保"正确的模型"运行在"正确的硬件"上服务"正确的应用"，从而让生成式AI部署在更广泛的平台范围内实现可持续和可访问。
...
TL;DR: PACE是一个无需训练的推理加速框架，通过统一的Condense-and-Extract范式同时加速视觉编码器和大型语言模型，在仅使用10%视觉token的情况下保留93.8%的性能并实现3.1倍的TTFT加速。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法存在两个根本性局限：一是大多仅在视觉编码器之后操作，未优化视觉编码阶段的延迟；二是在严格token预算下难以同时保留整体视觉上下文和细粒度细节，导致性能下降。

Method: 提出PACE框架，包含两个阶段：在Condense阶段，自适应像素压缩器（APC）在编码前评估视觉信息密度，自适应下采样冗余输入，减少编码器计算量；在Extract阶段，动态双注意力提取器（DDAE）融合编码器的内部视觉信号和LLM的语义信号来选择保留关键视觉token。

Result: 将PACE集成到Qwen2.5-VL-7B中，在仅使用10%视觉token的情况下保留了93.8%的原始性能，实现了3.1倍的首次token生成时间（TTFT）加速。

Conclusion: PACE通过统一的自适应压缩和动态提取机制，成功解决了视觉token剪枝中编码阶段未优化和视觉上下文与细节难以兼顾的问题，实现了高效的VLM推理加速。
...
TL;DR: CODE是一个统一推理时框架，通过跨模态联合置信度校准、不确定性引导的通用目标性增强和动态异常抑制三个互补组件，解决开放世界目标检测中基于多模态基础模型面临的语义歧义和刚性异常惩罚导致的已知/未知目标误判问题，在Real-World Detection基准上刷新SOTA。


<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测（OWOD）基于多模态基础模型时存在两个核心问题：一是单向文本到视觉匹配导致的语义歧义；二是刚性异常惩罚可能过度抑制靠近已知类决策边界的未知目标，导致未知目标被误判为已知或丢失。

Method: CODE包含三个互补组件：(1) Cross-Modal Joint Confidence Calibration：注入全局视觉原型来校准文本驱动的已知类预测；(2) Uncertainty-Guided Universal Objectness Enhancement：从局部视觉响应中度量分类犹豫程度，增强潜在未知目标；(3) Dynamic Outlier Suppression via Confidence Margin：用基于置信度边界的margin自适应调整替代刚性抑制，保留模糊的分布外实例。整个框架在推理时统一运作，无需重新训练。

Result: 在Real-World Detection基准上，以OWL-ViT L/14为骨干网络，CODE在Task 1中达到21.7 U-mAP（未知类）和40.8 K-mAP（已知类），分别超越此前SOTA 2.6和2.3个百分点。

Conclusion: CODE通过推理时跨模态校准和动态margin机制有效缓解了多模态OWOD中的语义歧义与未知目标过度抑制问题，验证了无需训练即可显著提升开放世界检测性能的可行性。
...
TL;DR: 提出一个自监督框架，从RGB-D观测中学习联合视觉空间表征，通过跨patch和patch内融合将深度几何先验与视觉骨干集成，在3D几何基准上优于同规模方法，同时保持语义迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现代视觉基础模型几乎只在RGB图像上训练，而许多具身系统可获取显式深度传感数据，其提供的几何信息是单目输入无法恢复的。现有方法未能有效利用深度几何先验来增强视觉表征。

Method: 采用自监督框架，将深度导出的几何先验与视觉骨干结合，通过inter-patch（跨patch）和intra-patch（patch内）两种融合机制编码外观与空间结构信息。

Result: 在多个3D几何基准上优于同规模先前方法，在标准RGB-D语义分割任务探测时保持竞争力，在3D感知上取得显著改进同时保留语义迁移能力。

Conclusion: 该框架证明深度几何先验可以有效融入自监督视觉表征学习，实现3D感知能力与语义迁移性能的平衡，为具身智能系统提供更优的视觉空间表征方案。
...
TL;DR: 本文提出UniFLM，一个统一的胎儿长骨自动分割与测量框架，并构建了高质量标注的FLB数据集，解决了产前超声检查中胎儿肢体骨骼发育评估的挑战。UniFLM通过语义感知跳跃连接、正采样策略和点回归映射模块，在分割精度和骨骼长度测量上优于现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 产前超声检查对胎儿肢体发育评估和先天畸形检测至关重要，但现有AI模型因缺乏高质量标注数据、缺乏统一的多长骨框架以及通用分割模型难以应对超声图像固有噪声和语义鸿沟等问题，常常忽略胎儿致死性骨骼发育不良的检测。

Method: 1) 构建FLB数据集，包含肱骨、股骨、胫腓骨、桡尺骨的高质量标注；2) 提出UniFLM统一框架，包含：语义感知跳跃连接模块（连接编码器与解码器的语义鸿沟）、正采样策略（自适应过滤噪声并提取关键语义信息）、点回归映射模块（学习临床医生标注模式以实现精确骨长测量）。

Result: 在FLB数据集上的大量实验表明，UniFLM在胎儿长骨评估中相比当前最先进模型取得了更优的准确性和更强的泛化能力。

Conclusion: UniFLM能有效应对超声图像的噪声与语义鸿沟问题，在胎儿长骨分割和测量任务中表现出优越性能，为产前胎儿骨骼发育评估提供了可靠的自动化工具。
...
TL;DR: 提出Sidecar，一个即插即用的语义增强模块，从初始角色描述中保留实体级信息并注入到后续提示嵌入中，在不训练、不修改基础扩散模型架构的情况下提升自由故事生成中角色一致性。


<details>
  <summary>Details</summary>
Motivation: 在自由形式故事生成中，角色仅在首次出现时被完整描述，后续仅以泛指提及或代词形式出现，导致后续提示丢失重要的身份相关语义，使角色一致性难以维持。

Method: Sidecar模块，从角色初始描述中提取实体级信息，并将缺失的语义注入到后续提示嵌入中；即插即用，无需额外训练，不修改基础扩散模型（SDXL和FLUX）架构。

Result: 在FreeStoryBench基准上，Sidecar在多个SDXL和FLUX基线上持续改善了提示-图像对齐度和角色一致性，且计算开销可忽略不计。

Conclusion: Sidecar通过语义增强有效解决自由故事生成中的角色一致性问题，具备通用性和零训练成本的优势，可广泛适用于现有扩散模型。
...
TL;DR: 本文提出了一种任务感知变形预测（TADP）方法用于单阶段3D目标检测，通过三重特征精炼聚合模块提取三级特征、多尺度特征聚合块进行尺度感知融合，并设计即插即用的任务感知变形头来感知各任务的侧重点和交互关系。在KITTI数据集上汽车mAP达到80.91%，超越多种现有方法。


<details>
  <summary>Details</summary>
Motivation: 单阶段3D目标检测器通常用同一组提取的特征完成不同任务（如分类、定位等），但将特征投影到一个对所有任务都自适应的公共空间是不可能的，这限制了各任务的性能表现。

Method: 1) 设计三重特征精炼聚合模块，自适应地提取三级特征；2) 设计多尺度特征聚合块，以尺度感知的方式融合多尺度特征；3) 提出即插即用的任务感知变形头，使每个任务的预测发生变形以感知各任务的侧重点和交互关系，并设计了三种不同的变形模块。

Result: 在KITTI数据集上，汽车mAP达到80.91%，超越了多种最先进的单阶段3D检测方法；同时验证了所提出的变形头在其他检测方法上也具有良好的迁移效果。

Conclusion: 任务感知变形预测方法有效解决了单阶段3D检测中特征无法同时适配所有任务的问题，通过为各任务定制变形预测提升了检测精度，且所设计的变形头具有良好的通用性。
...
TL;DR: 本文首次将圣诞树种植园测绘建模为罕见目标语义分割问题，提出硬负样本挖掘（HNM）策略以区分易混淆背景，并在监督测试、时间迁移和大规模验证三个层面评估。最佳模型DeepLabV3+ResNet-34在2020测试集上取得IoU 0.733、F1 0.846，HNM将PR-AUC从0.204提升至0.913，时间迁移亦表现良好。


<details>
  <summary>Details</summary>
Motivation: 圣诞树种植园在遥感领域尚未被充分探索，其测绘因高种植密度、短轮作周期、与周边植被视觉混淆、仅单一参考年有密集标签以及景观尺度上的严重类别不平衡而极具挑战。现有深度学习方法主要为森林、一般种植系统或果园设计，未明确针对圣诞树种植园的结构特异性和难负样本混淆问题。

Method: (i) 将圣诞树种植园测绘形式化为一种独特的罕见目标语义分割问题；(ii) 引入硬负样本挖掘（HNM）策略以提升与混淆背景模式之间的判别能力；(iii) 在监督测试、时间迁移和大规模验证等多个互补层面评估所提框架。

Result: 在保留的2020测试集上，最佳模型DeepLabV3（ResNet-34编码器）达到IoU 0.733、F1 0.846。HNM显著改善精确率-召回率行为，将PR曲线下面积从0.204提升至0.913。时间推断展现良好迁移性，2017/2018年IoU/F1为0.751/0.858，2023年为0.691/0.817。大规模验证表明任务固有难度：圣诞树种植园仅占评估区域极小比例，2017/2018年为1,498.4公顷（1.72%）、2023年为1,782.2公顷（2.04%），总评估面积为87,309.4公顷。

Conclusion: 本文成功证明了深度学习结合硬负样本挖掘策略能够有效应对圣诞树种植园测绘中罕见目标与难负样本混淆的核心挑战，在监督测试与时间迁移场景下均取得优异性能，但大规模验证也揭示了该类任务在极端类别不平衡条件下固有的困难。
...
TL;DR: 该论文提出R2M-Bench基准，通过同一次rollout内的相对校准（比较重访对与间隔匹配的非重访对及短程对）来区分视频世界模型中的"重访特定记忆一致性"与"通用时间稳定性"，避免绝对重访分数受渲染稳定性、重复内容和运动失败干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法用首帧与返回帧的绝对相似度评估视频世界模型的场景记忆，但高相似度可能只是因为rollout变化很小，而非模型真正记住了场景。这种模糊性使得绝对重访分数对渲染稳定性、重复内容和运动失败过于敏感，无法有效度量"重访选择性"的记忆能力。

Method: R2M-Bench对每个检测到的返回对，从同一rollout中选取两个对照组：间隔匹配的非重访对（度量通用时间稳定性）和短程对（估计短时一致性一致性）。由此计算MemoryGain（MG，重访相对时间基线的优势）和Normalized Memory Ratio（NMR，用短程到基线的动态范围归一化优势）。基准包含100个参考场景×3种"离开-返回"轨迹=300个实例，评估外观保真度、场景/物体身份、局部几何和持久状态等多个维度。

Result: 在7个动作条件视频世界模型上，Overall NMR与人类一致性判断的Spearman相关系数为ρ=0.547（95% CI [0.45,0.63]）。其与生成运动的组内相关性仅为0.072，而原始重访相似度为0.207，表明相对校准显著降低了慢动作捷径。DreamX-World-Memo在评估的模型中取得最高Overall NMR。

Conclusion: 同rollout相对校准是一种实用且有效的方法，能够区分重访特定的一致性记忆与通用时间稳定性，为视频世界模型的记忆评估提供了更可靠的度量框架。
...
TL;DR: 本文提出概率对齐（probabilistic alignment）概念，将视频生成模型视为世界模型时需要满足分布级对齐标准，并引入PAWBench基准和PAWEval评估协议，测试11个系统在50个场景中的生成分布与参考概率的一致性，发现当前模型均未达到概率对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成模型越来越多地被定位为世界模型，但物理过程在相同初始观测和动作下可能产生多种有效结果，因此世界模型不仅要生成单个合理轨迹，还要复现可能行为的分布。然而现有评估只关注单个视频的合理性，未检验重复生成是否能恢复正确的分布，缺乏分布级的评估标准。

Method: 作者将概率对齐形式化为世界模型的分布级准则，引入PAWBench基准，将重复视频生成rollout转换为可能的物理行为的经验分布；具体流程涵盖50个场景和11个当前系统，并进一步探究语言提示、初始噪声采样和模型训练是否可重塑模型的预测分布。

Result: 在50个场景和11个系统中，没有模型能同时匹配参考概率并覆盖有效的行为范围，表明当前视频生成模型与世界模型的概率对齐要求之间仍存在显著差距。语言提示、初始噪声采样或模型训练也未能完全消除这一差距。

Conclusion: 概率对齐是评估视频生成模型作为世界模型的必要标准，当前模型距离满足该标准仍有距离，本工作可作为未来向概率对齐世界建模努力的基础。
...
TL;DR: 本文提出KnockGS，一个交互响应式PhysicalGS框架，能从已知外力作用下的3D高斯物体动力学中估计其弹性和密度尺度，并将估计结果冻结回模拟器中，用于未拟合过的交互测试。


<details>
  <summary>Details</summary>
Motivation: 现有物理集成3D高斯表示方法可以模拟和渲染可变形的物体，但都假设材料参数已知或需人工指定，限制了其在需要从观测动力学中推断参数场景下的适用性。

Method: 将力诱导响应转化为校准信号：从观测动力学中提取时间响应特征，估计弹性尺度和密度尺度两个材料参数，然后将估计值冻结并写回同一模拟器，在未参与拟合的交互上进行验证。评估涵盖参数恢复精度（与隐藏真值比较）和响应级保真度（3D粒子轨迹、响应曲线统计、渲染帧质量）。

Result: 在五个留出材料目标上，该方法在尺度恢复精度上显著优于响应检索、全局回归和固定默认材料，且冻结的估计在方向和幅度不同的交互下仍保持预测性。

Conclusion: 交互响应包含足够信息来校准物理接地3D高斯表示中的材料尺度，该工作是迈向渲染外观与模拟响应一致的交互式PhysicalGS系统校准的第一步。
...
TL;DR: 提出SCG（Successive Capacity Growth）方法，让JEPA世界模型的编码器从极小的初始结构（28.3万参数）通过任务无关的测试-验证机制动态扩展宽度或深度，配合SIGReg正则化防止表示塌缩，在多个任务上显著提升预测损失、参数效率和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA世界模型采用固定尺寸的Vision Transformer编码器，其容量要么对于简单任务过度配置（造成冗余），要么对复杂任务配置不足（能力受限），且注意力头存在大量冗余。固定容量分配无法自适应任务复杂度。

Method: 1) SCG：从最小编码器开始（1头、2层、283K参数），通过函数保持（function-preserving）扩展在宽度（增加注意力头捕捉低层语义）或深度（增加Transformer块捕捉高层语义抽象）上增量增长；采用任务无关的test-and-verify机制试验架构变化并在预测损失未改善时回滚。2) SIGReg（Sketched Isotropic Gaussian Regularizer）：确保所有学习到的语义维度统计独立且与预测目标对齐，防止架构增长过程中出现表示塌缩。

Result: 在60维多目标动力学任务中，SCG自然触发深度扩展，预测损失比固定小基线提升20.3%，参数效率比扩展到固定大模型高出56倍；在2D导航任务中，单次宽度扩展即比固定大模型提升23%。在三个复杂度递增的环境中，自适应编码器均匹配或超越固定小基线，零假阳性扩展，函数保持精确（比率1.0，绝对差异0.0）。

Conclusion: JEPA世界模型编码器无需预先分配最大容量，可以随任务需求逐步增长，在保持表示质量的同时实现显著的计算和数据效率，为世界模型的架构自适应设计提供了新范式。
...
TL;DR: LeVJEPA是首个在LeJEPA无塌缩目标下训练的视频编码器，通过单一编码器+投影器架构和SIGReg正则化消除对目标编码器、stop-gradient及掩码重建的依赖，在显著降低预训练计算成本的同时匹配或超越现有视频表示学习方法，并证明视频可作为通用视觉预训练的可行甚至更优基板。


<details>
  <summary>Details</summary>
Motivation: 视频蕴含物理世界的时间结构，但从视频中学习表示历来计算代价高昂。现有自监督方法要么依赖复杂的架构不对称性（EMA目标编码器、stop-gradient、容量受限的预测器）来防止表示塌缩，要么通过像素空间掩码重建来规避塌缩，这些方法计算开销大且机制繁琐。作者旨在引入一个无需这些复杂设计的更简洁、高效的视频预训练框架。

Method: LeVJEPA采用单一编码器+投影器的极简架构，在无塌缩目标下训练。其核心是：（1）通过全局与局部视图的不变损失进行训练，并由SIGReg正则化提供可证明的防塌缩保证，消除了对EMA编码器、stop-gradient和预测器的依赖；（2）目标仅含单一超参数，大幅简化调参；（3）采用统一随机token丢弃（uniform random token dropping）来控制编码器观测的token数量，在降低计算成本的同时提升下游精度；（4）由于分支间无需不对称性，编码器可直接使用块因果注意力（block-causal attention）训练，使时间顺序成为编码器自身的属性。

Result: 在相同数据上匹配epoch数时，LeVJEPA在ViT-S/B/L等规模的模型上以5.6至20.8倍更少的预训练计算量匹配或超越V-JEPA 2。在匹配总FLOPs条件下，LeVJEPA在ImageNet-1K上超过最强视频基线7.6个点，在运动中心基准上保持竞争力。与计算匹配的、在相同视频帧上训练的DINOv2相比，LeVJEPA在外观中心评估上接近图像预训练编码器，同时在运动中心准确率上几乎翻倍。

Conclusion: 结果表明，一旦移除计算开销障碍，视频可以成为通用视觉预训练的可行甚至在某些方面更优的基板。LeVJEPA证明了一个无塌缩、无架构不对称的极简目标足以胜任视频表示学习，且视频提供的时序信号对运动理解等任务具有图像所不具备的优势。
...
TL;DR: MILO是一种利用大型重建模型（LRMs）的视觉能力，从单张图像中恢复详细3D人-物交互（3D HOI）的框架。其核心思想是利用LRM提供的几何支架保留人与物之间的相对排布和邻近线索，将重建问题转化为对LRM网格的解析：分割人/物组件、拟合参数化人体模型，并可选择性地将物体模板对齐到物体部分。


<details>
  <summary>Details</summary>
Motivation: 3D人-物交互（HOI）重建是AR/VR、机器人及具身AI等广泛应用中的基础问题，但深度模糊、遮挡和物体形状多样性使其极具挑战。现有方法主要依赖重投影和接触约束，将参数化人体模型和物体模板拟合到2D图像上，效果有限。作者希望探索一条不同的路径，利用LRM强大的几何重建能力来简化该问题。

Method: MILO框架的核心观察是LRM能提供保留人与物相对排布和邻近线索的强几何支架。该方法将3D HOI重建重构为对LRM网格的解读，流程包括：1）将LRM生成的网格分割为人体和物体组件；2）对人体部分拟合参数化身体模型；3）如有物体模板可用，则将其对齐到物体部分。这一框架显著简化了传统基于2D图像先验的重建流程。

Result: MILO在多个基准数据集和交互场景下取得了较强的重建精度，并超越了现有基线方法。

Conclusion: MILO证明了利用LRM的几何能力来简化3D HOI重建是有效的途径，展示了较高的重建精度和泛化能力，代码已开源。
...
TL;DR: 本文发现视觉语言模型（VLMs）中存在"视觉检索头"（Visual Retrieval Heads, VRHs）——约占全部注意力头的1.7-2.6%——这些头在因果上负责将文本描述与图像区域关联。通过统一的头评分设计空间，发现仅掩蔽前20个VRHs即可使定位精度下降高达80个百分点，而随机掩蔽同等数量的头几乎无影响。VRHs不仅复现了文本检索头的因果-稀疏-普适三联特征，还表现出跨任务泛化、功能特异性和架构共享性等新性质。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型能够根据文本提示定位图像区域并将视觉证据路由到输出，但其内部工作机制尚未被理解。受大语言模型中"检索头"（retrieval heads）概念的启发，作者提出疑问：VLMs是否也存在类似的视觉检索机制？这一问题的回答对于理解VLMs的可解释性和可控制性具有重要意义。

Method: 作者首先将现有的头部评分方法统一到一个设计空间中，涵盖三个维度：查询token选择、键聚合方式和跨样本聚合方式。通过比较发现，使用输出预测token的注意力分数，并对ground-truth参照区域进行求和，能最可靠地识别因果头部。基于此方法识别出VRHs，并在11个VLMs和5个指代表达基准上验证其因果作用。

Result: 实验表明：(1) VRHs约占全部注意力头的1.7-2.6%，掩蔽前20个VRHs可使接地精度下降最多80个百分点，而随机掩蔽几乎无影响；(2) VRHs能跨视觉参考任务泛化，对属性、空间、计数和视觉数学基准仍保持因果性；(3) VRHs功能上具有特异性，仅破坏定位能力而保留输出格式；(4) VRHs在架构上共享，可跨共享相同LLM主干但视觉编码器、投影器和指令微调不同的VLMs进行因果迁移。

Conclusion: 该工作首次证明了VLMs中存在与LLM文本检索头类比、但具有独特性质的视觉检索头机制，深化了对VLMs内部如何实现视觉-语言接地（grounding）功能的机理理解，并为后续干预和操控VLMs的视觉定位行为提供了精确的神经层面目标。
...
TL;DR: 本文提出UrbanGround——一个基于香港真实三维地理数据构建的物理约束城市沙盒，用于系统性评估多模态大模型（MLLM）智能体能否将局部城市感知转化为在真实尺度复杂城市中的可靠行动。研究发现当代MLLM智能体在视觉识别和短程空间推理方面表现尚可，但在方向感知、行人感知以及长时间探索中的目标导向行为组合方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型能够理解街景图像，但城市智能体（urban agency）的关键在于局部证据在智能体开始移动后是否仍然有用。现有研究缺乏一个能在真实城市尺度上测试"局部感知能否转化为可靠行动"的受控环境，因此需要一个可重复、可量化的测试平台来评估MLLM智能体的城市导航能力。

Method: 作者构建UrbanGround，这是第一个在由香港全域三维地理空间数据建造的物理约束复制城市中进行可测试实验的沙盒。该沙盒支持第一人称视角的闭环交互，并提供交互式地图用于导航，智能体可以直接进入三维城市从第一人称视角探索。实验设计通过三个递进的研究问题展开：(1) 智能体在主动观察后能否将局部场景充分接地以回答空间问题；(2) 当地标变远且更不明确时，这种接地能否支持导航；(3) 行为在面对路线变化和行人运动时是否仍能保持鲁棒性。

Result: 当代MLLM智能体在原子能力（视觉识别和短程空间推理）上通常表现有用，但方向感知和行人感知运动仍然不可靠。核心失败出现在扩展探索过程中：局部能力无法组合成持续的目线导向行为，且错误在缺乏有效纠正机制的情况下不断累积。

Conclusion: UrbanGround为研究MLLM智能体在复杂、开放端城市环境中探索可靠性的边界提供了平台，希望能支持更广泛的研究，以理解当前MLLM智能体在何种程度上能够可靠地进行城市探索。
